...
-- any workspace.world.visuals related stuff is just to ignore things where visual effects are.
...
function HitboxModule.RaycastModule(Player,ExtraData,SkillName,KeyName)
	...
	for Index = 1,#CharacterParts do
		...
		if Instances:IsA("BasePart") then
			...
			local EndPoint = (Character.HumanoidRootPart.CFrame * CFrame.new(0,0,-ExtraData.Size or 4)).Position;

			if ExtraData.Visualize then
				GlobalFunctions.Visualize(StartPoint,EndPoint,Color3.fromRGB(255, 57, 57))
			end
				...
				if Part:IsA("BasePart") and Part.Parent and Part.Parent:FindFirstChild("Humanoid") and Part.Parent:FindFirstChild("Humanoid").Health > 0 then
					...
					delay(.225,function()
						if ExtraData.Type and not ExtraData.DmgType == "Snake" then
							DamageManager.DeductDamage(Character,Part.Parent,SkillName,KeyName,{Type = ExtraData.Type, KeysLogged = ExtraData.KeysLogged})
						end
					end)
					return Hit
				else
...
function HitboxModule.RegionModule(Player,ExtraData,SkillName,KeyName)
	...
	local DetectionArea = workspace:FindPartsInRegion3WithIgnoreList(DetectionRegion,{workspace.World.Visuals,Character},50)
	for _,v in pairs(DetectionArea) do
					...
					if EnemyHumanoid.Health > 0 then
						delay(.225,function()
							if StateManager:Peek(EnemyHumanoid.Parent,"IFrame") then
								DamageManager.DeductDamage(Character,EnemyHumanoid.Parent,SkillName,KeyName,{Type = ExtraData.Type, KeysLogged = ExtraData.KeysLogged, SecondType = SecondType})
							end						
						end)
...
function HitboxModule.GetTouchingParts(Character,ExtraData,SkillName,KeyName)
	...
	local IsHit, HitOnce, EntitieList = false, false, {}
	for _,Hit in ipairs(Zone) do
		if not Hit:IsDescendantOf(Character) and not Hit:IsDescendantOf(workspace.World.Visuals) and Hit.Parent and Hit.Parent:FindFirstChild("Humanoid") then
			...
			if Victim ~= Character and EnemyHumanoid.Health > 0 and not table.find(EntitieList,Victim) and Victim.Parent == workspace.World.Live then
				if HitOnce then return end
				delay(.225,function()
					DamageManager.DeductDamage(Character,Victim,SkillName,KeyName,{Type = ExtraData.Type, KeysLogged = ExtraData.KeysLogged, SecondType = SecondType})
				end)
				Debris:AddItem(Hitbox,ExtraData.ExistTime)

				EntitieList[#EntitieList + 1] = Victim
				if ExtraData.HitOnce then
					HitOnce = true
				end
				IsHit = true
			end
...
--[[for _,Humanoids in ipairs(EnemyHumanoidList) do
		return IsHit,Humanoids
	end ]]
end

function HitboxModule.MagnitudeModule(Character,ExtraData,SkillName,KeyName,Ting)
	local Hits = {}
	...
	for _,Characters in ipairs(workspace.World.Live:GetChildren()) do
		if Characters:FindFirstChild("HumanoidRootPart") and Characters:FindFirstChild("Humanoid") then
			local Calculation = (HumanoidRootPart.Position + (HumanoidRootPart.CFrame.lookVector * 3.5) - Characters.HumanoidRootPart.Position).Magnitude
	...
	for _,EnemyCharacter in ipairs(Hits) do
		...
		delay(ExtraData.Delay or .225,function()
			if StateManager:Peek(EnemyCharacter,"IFrame") and Ting == nil then
				DamageManager.DeductDamage(Character,EnemyCharacter,SkillName,KeyName,{Type = ExtraData.Type, KeysLogged = ExtraData.KeysLogged, SecondType = SecondType})
			end
		end)
