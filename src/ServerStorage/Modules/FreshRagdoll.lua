...
Ragdoll.StartRagdoll = function(Character, Type)
    if Character and Character:FindFirstChild("Ragdoll") then
        return
    end
    if StateManager:Peek(Character, "Blocking") then
        return
    end
    if not StateManager:Peek(Character, "IFrame") then
        return
    end
...
Ragdoll.EndRagdoll = function(Character)
    ...
    HumanoidRootPart.CFrame = CFrame.new(HumanoidRootPart.Position + Vector3.new(0, 1.75, 0))
