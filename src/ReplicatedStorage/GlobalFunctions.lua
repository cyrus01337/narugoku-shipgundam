...
function GlobalFunctions.TweenFunction(ObjectData, InfoData)
    ...
    local Tween = TweenService:Create(
        ObjectData["Instance"],
        TweenInfo.new(ObjectData["Duration"], ObjectData["EasingStyle"], ObjectData["EasingDirection"], 0, false, 0),
        InfoData
    )
    Tween:Play()
    Tween:Destroy()
...
function GlobalFunctions.CheckDistance(Player, Distance)
    ...
    if
        Range < (Character:FindFirstChild("HumanoidRootPart").CFrame.Position - CurrentCamera.CFrame.Position).Magnitude
    then
        return false
    end
    return true
