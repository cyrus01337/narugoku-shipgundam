...
return function(Data)
	...
	for _ = 1, 5 do
		...
		local Expand = TweenService:Create(Shockwave, TweenInfo.new(.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = Vector3.new(RandomSize, .5, RandomSize)})
		Expand:Play()
		Expand:Destroy()

		local Spin = TweenService:Create(Shockwave, TweenInfo.new(.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, -1), {Orientation = Shockwave.Orientation + Vector3.new(0,360,0)})
		Spin:Play();
		Spin:Destroy()

		local Pillow = TweenService:Create(Shockwave, TweenInfo.new(.35 + math.random(), Enum.EasingStyle.Linear, Enum.EasingDirection.Out, 0, false, 0), {Position = Shockwave.Position + Vector3.new(0, math.random(5,50),0), Transparency = 1})
		Pillow:Play();
		Pillow:Destroy()
	...
	for Index = 1,math.random(6,8) do
		...
		local Tween = TweenService:Create(Orbie,OrbieTweenInfo,{CFrame = CFrame.new(Start,End)*CFrame.new(0,0,-(math.random(2,5) * 35)),Size = Vector3.new(0,0,24)})
		Tween:Play()
		Tween:Destroy()
	...
	for Index = 1,2 do			
		...
		local Tween = TweenService:Create(Ring,RingTween,{CFrame = Ring.CFrame * CFrame.new(0,45,0) ,Size = Vector3.new(0,0,0)})
		Tween:Play()
		Tween:Destroy()
