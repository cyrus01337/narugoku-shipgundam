...
return function(Data)
	...
	for Index = 1, 5 do
		...
		coroutine.wrap(function()
			wait(.25)
			local Tween = TweenService:Create(Rock, TweenInfo.new(.5, Enum.EasingStyle.Sine, Enum.EasingDirection.Out, 0, false, 0), {["Size"] = Vector3.new(0, 0, 0)})
			Tween:Play()
			Tween:Destroy()
		end)()
		Debris:AddItem(Rock, .75)
	end
	...
	local Tween = TweenService:Create(Ball, TweenInfo.new(.4, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {["Transparency"] = 1})
	Tween:Play()
	Tween:Destroy()

	Debris:AddItem(Ball,3)
	
	local _ = not Spear.Name == "Bird" and VfxHandler.FloorFreeze(CFrame.new(Result.Position) * CFrame.new(0, 5, 0), 100, 10, 15, nil, 10,Vector3.new(math.random(20,25), 0.989, math.random(20,25)),Character)
