...
return function(Data)
    ...
    local Tween = TweenService:Create(
        ...
        { Transparency = 1, Size = Vector3.new(50, 50, 50), CFrame = circle2.CFrame * CFrame.fromEulerAnglesXYZ(0, 1, 0) }
    )
    Tween:Play()
    Tween:Destroy()
    ...
    local Tween = TweenService:Create(
        ...
        { Size = Vector3.new(55, 2, 55), Transparency = 1 }
    )
    Tween:Play()
    Tween:Destroy()
    ...
    for _ = 1, 4 do
        ...
        local CFrameIndex = CFrame.new(Data.Position)
            * CFrame.new(math.random(-SizeRange, SizeRange), 0, math.random(-SizeRange, SizeRange))
        ...
        local Tween = TweenService:Create(
            ...
            {
                Transparency = 0,
                Size = Vector3.new(SizeIndex, SizeIndex, SizeIndex),
                CFrame = Smoke.CFrame * CFrame.Angles(math.random(360), math.random(360), math.random(360)),
            }
        )
        Tween:Play()
        Tween:Destroy()
    ...
    for _ = 1, math.random(2, 4) do
        ...
        local B7 = TweenInfo.new(0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.Out, 0, false, 0)
        ...
        local Tween = TweenService:Create(
            ...
            {
                ["Transparency"] = 1,
                ["CFrame"] = SlashEffect.CFrame * CFrame.Angles(math.pi * AddVector, 0, 0),
                ["Size"] = SlashEffect.Size + Vector3.new(0, SizeAdd, SizeAdd),
            }
        )
        Tween:Play()
        Tween:Destroy()

        Debris:AddItem(SlashEffect, 0.12)
    end
    ...
    local A1 = TweenInfo.new(0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.Out, 0, true, 0)
    ...
    local Tiasd = TweenInfo.new(0.04, Enum.EasingStyle.Sine, Enum.EasingDirection.Out, 0, true, 0)
    ...
    for Index = 1, math.random(5, 6) do
        ...
        local X, Y, Z =
            math.rad(math.random(1, 24) * 15), math.rad(math.random(1, 24) * 15), math.rad(math.random(1, 24) * 15)

        Part.CFrame = CFrame.new(Data.Position) * CFrame.Angles(X, Y, Z)
        Part.Parent = DistanceThudnerFolder

        local A3 = TweenInfo.new(0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.Out, 0, false, 0)
