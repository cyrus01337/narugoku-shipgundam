...
function CreateLightning(Start,End,numberofparts)
	...
	for Index = 1,numberofparts do
		...
		local Ti3 = TweenInfo.new(.25,Enum.EasingStyle.Linear,Enum.EasingDirection.Out,4,true,0)
...
return function(Data)
	...
	coroutine.resume(coroutine.create(function()
		...
		circleslash.Parent = workspace.World.Visuals
	...
	coroutine.resume(coroutine.create(function()
		for _ = 1,math.random(5,6) do
			...
			local x,y,z = math.rad(math.random(1,12) * 30),math.rad(math.random(1,12) * 30),math.rad(math.random(1,12) * 30)
			P.CFrame = CFrame.new(Root.Position) * CFrame.Angles(x,y,z)
			P.Parent = workspace.World.Visuals
			local A3 = TweenInfo.new(.2,Enum.EasingStyle.Linear,Enum.EasingDirection.Out,0,false,0)
	...
	LightningExplosion.new(Root.Position, .35, 12, ColorSequence.new(Color3.fromRGB(255, 251, 134)), ColorSequence.new(Color3.fromRGB(117, 237, 255), Color3.fromRGB(255, 251)), nil)

	for Index = 1,2 do			
		local Ring = ReplicatedStorage.Assets.Effects.Meshes.myring:Clone()
		Ring.Size = Vector3.new(50,3,50)
		Ring.Material = Enum.Material.Neon
		Ring.BrickColor = BrickColor.new(Index % 2 == 0 and "Pastel blue-green" or "Yellow")
		...
		local Tween = TweenService:Create(Ring,RingTween,{["Transparency"] = 1, ["CFrame"] = Ring.CFrame * CFrame.new(0,15,0) ,["Size"] = Vector3.new(0,0,0)})
		Tween:Play()
		Tween:Destroy()
