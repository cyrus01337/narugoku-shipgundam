...
return function(Data)
    ...
    DustParticles.FallingDust.Size =
        NumberSequence.new({ NumberSequenceKeypoint.new(0, Size / 2), NumberSequenceKeypoint.new(1, Size / 2) })
    DustParticles.FallingDust.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Result.Instance.Color),
        ColorSequenceKeypoint.new(1, Result.Instance.Color),
    })

    DustParticles.RockParticles.Size =
        NumberSequence.new({ NumberSequenceKeypoint.new(0, Size / 12), NumberSequenceKeypoint.new(1, Size / 12) })
    DustParticles.RockParticles.Speed = NumberRange.new(Size, Size)
    DustParticles.RockParticles.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Result.Instance.Color),
        ColorSequenceKeypoint.new(1, Result.Instance.Color),
    })
    ...
    Debris:AddItem(RegularBall, 0.2)
    ...
    coroutine.resume(coroutine.create(function()
        for Index = 1, 2 do
            ...
            RingTing.Rotation = Vector3.new(
                math.random(-360, 360) * Index,
                math.random(-360, 360) * Index,
                math.random(-360, 360) * Index
            )
            RingTing.Size = Vector3.new(10, 1, 10)

            local _ = (Index and Index == 1 and RingTing.BrickColor and BrickColor.new("Daisy orange"))
                or (Index and Index == 3 and RingTing.BrickColor and BrickColor.new("Institutional white"))
            RingTing.Parent = workspace.World.Visuals
            Debris:AddItem(RingTing, 0.35)
    ...
    for _ = 1, 2 do
        ...
        local Tween = TweenService:Create(
            Orbie,
            OrbieTweenInfo,
            { CFrame = CFrame.new(Start, End) * CFrame.new(0, 0, -(math.random(2, 5) * 18)), Size = Vector3.new(
                0,
                0,
                30
            ) }
    ...
    if Data.Index == 2 then
        coroutine.resume(coroutine.create(function()
            for _ = 1, 2 do
                ...
                local Tween = TweenService:Create(
                    Ring,
                    RingTween,
                    { CFrame = Ring.CFrame * CFrame.new(0, 20, 0), Size = Vector3.new(0, 0, 0) }
                )
                Tween:Play()
                Tween:Destroy()
        ...
        for _ = 1, 2 do
            ...
            FireMagicParticle.Size = NumberSequence.new({
                NumberSequenceKeypoint.new(0, math.random(4, 6)),
                NumberSequenceKeypoint.new(1, 0),
            })
            FireMagicParticle.Transparency =
                NumberSequence.new({ NumberSequenceKeypoint.new(0, 1), NumberSequenceKeypoint.new(1, 0.25) })
            FireMagicParticle.Acceleration = Vector3.new(0, 0, 0)
            FireMagicParticle.Lifetime = NumberRange.new(0.5)
        ...
        for _ = 1, 3 do
            ...
            Debris:AddItem(ShockwaveTing, 0.35)
            wait()
        end
    end
end
