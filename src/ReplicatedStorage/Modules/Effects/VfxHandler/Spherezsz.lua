...
return function(Data)
    ...
    for Index = 1, Data.Amount do
        ...
        local End = Cframe
            * CFrame.new(math.random(-Range, Range), math.random(-Range, Range), math.random(-Range, Range))
        ...
        local Tween = TweenService:Create(
            Sphere,
            TweenInfo.new(TweenDuration1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
            {
                ["CFrame"] = CFrame.new(End.p, Cframe.p) * CFrame.Angles(math.rad(-90), 0, 0),
        ...
        local MeshTween = TweenService:Create(
            ...
            TweenInfo.new(TweenDuration1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
            {
                ["Scale"] = Vector3.new(RandomCalculation, MagnitudeCalculation, RandomCalculation),
                ["Offset"] = Vector3.new(0, -MagnitudeCalculation / 2, 0),
            }
        )
        MeshTween:Play()
        MeshTween:Destroy()
        ...
        coroutine.resume(coroutine.create(function()
            ...
            if Sphere:findFirstChild("Mesh") then
                local EndTween = TweenService:Create(
                    Sphere.Mesh,
                    TweenInfo.new(TweenDuration2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                    { ["Scale"] = Vector3.new(0, 0, 0), ["Offset"] = Vector3.new(0, -MagnitudeCalculation, 0) }
                )
                EndTween:Play()
                EndTween:Destroy()
            end
