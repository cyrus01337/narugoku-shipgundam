...
return function(Cframe, ToZ, ToX, Duration, Color, Width0, Width1, LightEmission, Texture, Transparency, TextureLength, TextureSpeed)
	
	local BeamShock = script.BeamShock:Clone()
	BeamShock.Parent = workspace.World.Visuals
	BeamShock.CFrame = Cframe
	Debris:AddItem(BeamShock, Duration + .1)
	...
	local Tween = TweenService:Create(Attachment11,TweenInfo.new(Duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out,0,false,0), {Position = Vector3.new(0, ToZ, -ToX)})
	Tween:Play()
	Tween:Destroy()

	local Tween = TweenService:Create(Attachment21,TweenInfo.new(Duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out,0,false,0), {Position = Vector3.new(0, ToZ, ToX)})
	Tween:Play()
	Tween:Destroy()

	local Tween = TweenService:Create(BeamShock.Beam,TweenInfo.new(Duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out,0,false,0), {
	...
	local Tween = TweenService:Create(BeamShock.Beam2,TweenInfo.new(Duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out,0,false,0), {
		CurveSize0 = CurveCalc, 
		CurveSize1 = -CurveCalc, 
		Width0 = 0, 
		Width1 = 0
