...
return function(Data)
	...
	for _ = 1, Quantity do
		...
		local NewPos = Pos + Vector3.new(math.random(Offsets.X[1], Offsets.X[2]), math.random(Offsets.Y[1], Offsets.Y[2]), math.random(Offsets.Z[1], Offsets.Z[2]))
		Orb.CFrame = CFrame.lookAt(NewPos, Pos)
		Orb.Parent = workspace.World.Visuals
		Goal.CFrame = Orb.CFrame * CFrame.new(0,0,math.random(Offsets.Offset[1], Offsets.Offset[2]))
		
		local Animate = TweenService:Create(Orb, TweenInfo, Goal)
		Animate:Play()
		Animate:Destroy()
