...
return function(Data)
    ...
    Debris:AddItem(RockModel, RocksLength + 0.25)

    for Index = 1, Amount do
        --// Calculations
        local SizeCalc = math.sin(math.pi / Amount) * Iteration * 2 * 1.01
        local PositionCalc = Cframe
            * CFrame.Angles(0, math.rad(Index * Angeled360), 0)
            * CFrame.new(0, 0, Iteration)
        ...
        local RaycastResult = workspace:Raycast(
            RaycastToSet.Position,
            RaycastToSet.UpVector * (-RandomizedForCalc * 2),
            RayParam
        ) or {}
        ...
        if Target then
            ...
            InstancedPart.CFrame = Cframe
                * CFrame.Angles(0, math.rad(Index * Angeled360), 0)
                * CFrame.new(0, 0, Iteration / 2)
            ...
            local Tween = TweenService:Create(
                ...
                TweenInfo.new(FirstDuration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, 0, false, 0),
                {
                    CFrame = CFrame.new(
                        ToReturn.Position,
                        CFrame.new(X, Y, Z, A1, A2, A3, A4, A5, A6, A7, A8, A9).Position
            ...
            coroutine.resume(coroutine.create(function()
                ...
                if InstancedPart then
                    ...
                    local Tween = TweenService:Create(
                        ...
                        { CFrame = InstancedPart.CFrame * CFrame.new(0, -Max * 1.25, 0) }
                    )
                    Tween:Play()
                    Tween:Destroy()
                end
