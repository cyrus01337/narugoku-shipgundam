...
Methods:
	...
	RotatedRegion3:FindPartsInRegion3WithInclude(Instance Array Include, Integer maxParts)
		> returns array of parts in the RotatedRegion3 object
		> will return a maximum number of parts in array [maxParts] the default is 20
		> parts that either are descendants of the [Include array] or actually are the [Include array] instances are all that will be checked
...
function RotatedRegion3:FindPartsInRegion3WithInclude(Include, maxParts)
	Include = Include or {}
	local found = {}
	local parts = game.Workspace:FindPartsInRegion3WithInclude(self.AlignedRegion3, Include, maxParts)
	for i = 1, #parts do
		if (self:CastPart(parts[i])) then
			table.insert(found, parts[i])
		end
