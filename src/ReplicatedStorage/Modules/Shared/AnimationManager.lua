...
function AnimationHandler.LoadAnimations(Character, ClassNames)
    ...
    for _, ClassName in ipairs(ClassNames) do
        ...
        for _, Animation in ipairs(CachedShared) do
            if AnimationHandler[Character.Name].LoadedAnimations[Animation.Name] then
                continue
            end
...
function AnimationHandler.PlayAnimation(AnimationName, AnimationData)
    ...
    if not AnimationHandler[Character.Name].LoadedAnimations[AnimationName] then
        warn(AnimationName, "is not a valid animation")
        return
    end

    local FadeTime = AnimationData and AnimationData.FadeTime or 0.1
...
function AnimationHandler.ClearAnimations(Character)
    ...
    if HasProfile == nil or AnimationHandler.LoadedAnimations == nil then
        return
    end
