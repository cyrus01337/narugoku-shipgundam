...
function SpeedHandler.changeSpeed(Character, Speed, Duration, Priority, Trail, UnequipSound)
    ...
    task.spawn(function()
        ...
        while
            not StateHandler:Peek(Character, "Speed") --[[or SpeedData.Priority <= Priority]]
        do
            local SpeedData = StateHandler:ReturnData(Character, "Speed")
...
function SpeedHandler.HideSword(Character, Sound)
    if Character:FindFirstChild("AokijiSword") or not Character:FindFirstChild("SwordEffect") then
        return
    end
    ...
    if RunService:IsClient() then
        ...
        if
            StateHandler:Peek(Character, "Attacking")
            and StateHandler:Peek(Character, "Guardbroken")
            and Sound == nil
        then
