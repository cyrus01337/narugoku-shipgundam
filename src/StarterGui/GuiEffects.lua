...
--|| Tweens ||--
local tasdasdi = TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, 0, false, 0)
local PunchEffectTweens = {
    First = TweenInfo.new(0.2, Enum.EasingStyle.Linear, Enum.EasingDirection.Out, 0, false, 0),
    Second = TweenInfo.new(0.2, Enum.EasingStyle.Linear, Enum.EasingDirection.Out, 0, false, 0),
}
...
function module:Slash(Parent, Position)
    if CharacterInfo[_G.Data.Character]["Combat"] then
        ...
        local Tween = TweenService:Create(
            F1,
            PunchEffectTweens.First,
            {
                Size = UDim2.new(0.3, 0, 0.3, 0),
                ImageTransparency = 1,
                Rotation = math.random(-3, 3) * 10,
...
function module.ClickEffect(Frame, Color, growsize)
    if Frame then
        ...
        local Tween = TweenService:Create(
            Circle,
            tasdasdi,
            { Size = UDim2.new(growsize or 6, 0, growsize or 6, 0), ImageTransparency = 1 }
        )
        Tween:Play()
        Tween:Destroy()

        Debris:AddItem(Circle, 0.25)
    end
end
