...
local function UpdateUi()
    ...
    if string.len(ComboIndicator.Text) < 3 then
        return
    end
    ...
    for _, Child in ipairs(ComboList) do
        if string.sub(string.lower(Child), 1, string.len(ComboIndicator.Text)) == string.lower(ComboIndicator.Text) then
            if string.len(string.sub(Child, string.len(ComboIndicator.Text))) == 0 then
                return
            end
...
local ComboIndicator = {
    ["Initiate"] = function(UiData)
        ...
        CurrentIndicatorData[#CurrentIndicatorData + 1] =
            { ComboIndicator = ComboIndicator, OriginalTime = OriginalTime }
        --RemoveIndicator(
    end,
...
Runservice.Heartbeat:Connect(function(DeltaTime)
    if #CurrentIndicatorData == 0 then
        return
    end
    if ComboIndicator.Text == "" then
        return
    end
    if ComboIndicator == nil or ComboText == nil then
        return
    end
