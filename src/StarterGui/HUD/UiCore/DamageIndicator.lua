	...
	["AddText"] = function(UiData)
		if UiData.Number ~= 0 then
			if script.Parent.Parent:FindFirstChild("EffectFrame") then
				local Tween = TweenService:Create(script.Parent.Parent.EffectFrame,Ti3,{Size = UDim2.new(0.134, 0,.186, 0)})
				Tween:Play()
				Tween:Destroy()
			end
			...
			wait(.35)
			if script.Parent.Parent:FindFirstChild("EffectFrame") then
				local Tween = TweenService:Create(script.Parent.Parent.EffectFrame,Ti3,{Size = UDim2.new(0,0,0,0)})
				Tween:Play()
				Tween:Destroy()
			end
		end
