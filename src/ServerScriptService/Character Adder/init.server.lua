...
local function AddToEntites(Character)
	...
	AnimationRemote:FireClient(Players:GetPlayerFromCharacter(Character),Character,"LoadAnimations",ReplicatedStorage.Assets.Animations.Shared:GetDescendants())
	--ProfileService:Replicate(Players:GetPlayerFromCharacter(Character))	
end
...
Players.PlayerAdded:Connect(function(Player)
	...
	GlobalFunctions.NewInstance("BoolValue",{Parent = Player:WaitForChild("Mode"), Name = "ModeBoolean", Value = false})
		...
		if MarketPlaceService:UserOwnsGamePassAsync(Player.UserId, 18729033) then
			for Index,Table in next, Messages do
				if Message == "/e "..Index and StateManager:Peek(Character,"Emoting") then
					...
					StateManager:ChangeState(Character,"Emoting",.001)
				end
			end
		end
	end)
