...
function HitboxModule.RaycastModule(Player,ExtraData,SkillName,KeyName)
	...
	for Index = 1,#CharacterParts do
		...
		if Instances:IsA("BasePart") then
			...
			local _ = ExtraData.Visualize and GlobalFunctions.Visualize(StartPoint,EndPoint,Color3.fromRGB(255, 57, 57))
				...
				if Part:IsA("BasePart") and Part.Parent and Part.Parent:FindFirstChild("Humanoid") and Part.Parent:FindFirstChild("Humanoid").Health > 0 then
					...
					delay(.225,function()
						if ExtraData.Type and not ExtraData.DmgType == "Snake" then
							DamageManager.DeductDamage(Character,Part.Parent,SkillName,KeyName,{Type = ExtraData.Type, KeysLogged = ExtraData.KeysLogged, Damage = ExtraData.Damage})
						end
					end)
					return Hit
				else
					...
					if EnemyHumanoid.Health > 0 then
						delay(.225,function()
							--if StateManager:Peek(EnemyHumanoid.Parent,"IFrame") then
								DamageManager.DeductDamage(Character,EnemyHumanoid.Parent,SkillName,KeyName,{Type = ExtraData.Type, KeysLogged = ExtraData.KeysLogged})
							--end						
						end)
...
function HitboxModule.MagnitudeModule(Character,ExtraData,SkillName,KeyName,Ting,Ting2)
	...
	for _,EnemyCharacter in ipairs(Hits) do
		...
		delay(ExtraData.Delay or .225,function()
			if  Ting == nil then
				DamageManager.DeductDamage(Character,EnemyCharacter,SkillName,KeyName,{Type = ExtraData.Type, KeysLogged = ExtraData.KeysLogged, SecondType = SecondType},Ting2)
			end
		end)
...
function HitboxModule.ConeDetection(Character, ExtraData, SkillName, KeyName,Ting)
	...
	for _,Characters in ipairs(workspace.World.Live:GetChildren()) do
		if Characters:FindFirstChild("HumanoidRootPart") and Characters:FindFirstChild("Humanoid")  then
			...
			local Calculation = (HumanoidRootPart.Position + (HumanoidRootPart.CFrame.lookVector * 3.5) - Characters.HumanoidRootPart.Position).Magnitude
			if Calculation <= ExtraData.Range and DotVector >= (ExtraData.Angle or -.5) then
				if Characters ~= Character then
					Hits[#Hits + 1] = Characters
					ExtraData.Range = Calculation
				end
