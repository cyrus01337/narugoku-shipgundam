...
local Combat = {
    ...
    ["Run"] = {
        ["Execute"] = function(Player, CharacterName, KeyData, MoveData, ExtraData)
            ...
            if not StateManager:Peek(Character, "Emoting") then
                return
            end
            ...
            while StateManager:Peek(Character, "Running") do
                ...
                if
                    ...
                    or not StateManager:Peek(Character, "Attacking")
                    or StateManager:Peek(Character, "Stunned")
                    or SkillData.Skill == "Swing"
                    or not StateManager:Peek(Character, "Guardbroken")
                then -- or SkillData.Skill == "Block" then
    ...
    ["Block"] = {
        ...
        ["Execute"] = function(Player, CharacterName, KeyData, MoveData, ExtraData)
            ...
            StateManager:ChangeState(
                ...
                "Blocking",
                true,
                { AllowedSkills = { ["Dash"] = true }, IsBlocking = true }
            )
            StateManager:ChangeState(Character, "Attacking", 0, { AllowedSkills = { ["Dash"] = true } })

            local WhichRunAnimation = HasCombat and "Running" or "SwordRunning"

            local _ = StateManager:Peek(Character, "Running")
                and AnimationRemote:FireClient(Player, WhichRunAnimation, "Stop")
                and Character:FindFirstChild("Running")
                and Character.Running:Destroy()
            ...
            coroutine.wrap(function()
                ...
                local _ = StateManager:Peek(Character, "Running")
                    and AnimationRemote:FireClient(Player, WhichRunAnimation, "Play", { Looped = true })
                    and GlobalFunctions.NewInstance("BoolValue", { Parent = Character, Name = "Running" })
            end)()
    ...
    ["Mode"] = function(Player, CharacterName, KeyData, MoveData, ExtraData)
        ...
        CharacterModules[Data.Character .. "Mode"]["Transformation"](
            Player,
            CharacterName,
            KeyData,
            MoveData,
        ...
        local DashCooldown = Data.Character == "Killua" and 0.5 or 0.5
        ...
        while Character and Humanoid.Health >= 1 and Data.Character == CachedCharacter do
            ...
            if ModeData and ModeNumber.Value <= 0 or not ModeData then
                StateManager:ChangeState(Character, "Mode", 0.01)
                break
            end
            wait(1)
        end
    ...
    ["Swing"] = function(Player, CharacterName, KeyData, MoveData, ExtraData)
        ...
        PlayerCombo.ComboVariation = ExtraData.WhichButton == "L" and PlayerCombo.ComboVariation .. "L"
            or PlayerCombo.ComboVariation .. "R"
        ...
        GUIRemote:FireClient(Player, "ComboIndicator", {
            ...
            Function = (PlayerCombo.ComboVariation == "L" or PlayerCombo.ComboVariation == "R") and "Initiate"
                or "AddIndicator",
            Variation = PlayerCombo.ComboVariation,
        })

        local _ = string.len(PlayerCombo.ComboVariation) >= 5
            and StateManager:ChangeState(Player.Character, "Guardbroken", 1.35)
