...
return function(Player,CharacterName,KeyData,MoveData,ExtraData)
	...
	local AdjustAnimConfigurations = (Type == "Sword" and AdjustSpeed and .75) or (Type == "Combat" and AdjustSpeed and 1)
	...
	local _ = (AirCheck and HasStand == nil and AnimationRemote:FireClient(Player, CharacterAnimationType..Side..NumberEvaluation, "Play")) or (not AirCheck and HasStand == nil and AnimationRemote:FireClient(Player, AnimCombo..NumberEvaluation, "Play",{AdjustSpeed = AdjustAnimConfigurations}) or HasStand and HasStand.Humanoid:LoadAnimation(CombatAnims[CharacterAnimationType..Side..NumberEvaluation]):Play())
		
	MoveStand.MoveStand(Character, {Priority = 1, Duration = .55})
	...
	StateManager:ChangeState(Character, "Attacking", .225)
	...
	coroutine.wrap(function()
		if Variations[PlayerCombo.ComboVariation] == nil then
			local HitResult,HitObject = HitboxModule.MagnitudeModule(Character, {Range = ExtraData.HitboxRange or 5, KeysLogged = PlayerCombo.KeysLogged, Type = Type}, KeyData.SerializedKey, CharacterName)
			if HitResult then
	...
	SpeedManager.changeSpeed(Character,7,.8,2)
