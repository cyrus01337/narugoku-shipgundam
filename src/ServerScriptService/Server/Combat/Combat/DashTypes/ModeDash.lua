...
return function(Character, ExtraData, MoveData, DashCopy)
    ...
    NetworkStream.FireClientDistance(
        ...
        "ClientRemote",
        150,
        { Character = Character, Module = "PlayerClient", Function = "ModeDash" }
    )

    StateManager:ChangeState(Character, "Dashing", 0.5)

    VfxHandler.PlayerInvis({ Character = Character, Duration = StateManager:Peek(Character, "Blocking") and 0.3 or 0.5 })

    SpeedManager.changeSpeed(Character, StateManager:Peek(Character, "Blocking") and 55 or 65, 0.5, 0.3, false)
    wait(StateManager:Peek(Character, "Blocking") and 0.3 or 0.5)
