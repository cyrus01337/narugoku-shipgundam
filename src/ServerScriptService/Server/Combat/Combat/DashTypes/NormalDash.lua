...
return function(Character,ExtraData,MoveData,DashCopy)
	...
	local AnimationDirection = DirectionKey == "W" and "FrontDash" or DirectionKey == "A" and "LeftDash" or DirectionKey == "D" and "RightDash" or "BackDash"
	
	if StateManager:Peek(Character,"Emoting") and not StateManager:Peek(Character, "Blocking") or StateManager:Peek(Character,"Running") or not (SkillData.Skill == "Swing") then
	...
	NetworkStream.FireClientDistance(Character, "ClientRemote", 150, {Character = Character , Module = "PlayerClient", Function = "Dash"})
	...
	BodyVelocity.Velocity = CFrame.new(Root.CFrame.Position, MoveDirection + Root.CFrame.Position).LookVector * DashingForce
