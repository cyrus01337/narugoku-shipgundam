...
local Tanjiro = {
	["FirstAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		local NumberEvaluation = (PlayerCombo.Hits < 5 and TimeEvaluation and PlayerCombo.Hits + 1) or (PlayerCombo.Hits > 5 and 1) or (not PlayerCombo.TimeEvaluation and 1)

		SpeedManager.changeSpeed(Character,8,1,1)
		StateManager:ChangeState(Character,"Guardbroken",.8)				

		wait(.25)
		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "TanjiroVFX", Function = "WaterStuff", Duration = .2})
		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "TanjiroVFX", Function = "Trail", Sword = Sword, Duration = .5})

		CameraRemote:FireClient(Player, "CameraShake", {FirstText = 8, SecondText = 10})
		PlayerCombo.KeysLogged = NumberEvaluation

		HitboxModule.GetTouchingParts(Player,{ExistTime = 2, Type = "Sword", KeysLogged = NumberEvaluation, Size = Vector3.new(8.999, 4.517, 31.284), Transparency = 1, PositionCFrame = Root.CFrame * CFrame.new(0,0,-15)},KeyData.SerializedKey,CharacterName)
		...
		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "TanjiroVFX", Function = "WaterSurfaceSlash", Sword = Sword})
	end,
	...
	["ThirdAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		AnimationRemote:FireClient(Player,"Whirlpool","Play",{AdjustSpeed = .75})
		delay(.125,function()
		...
		StateManager:ChangeState(Character,"Guardbroken",.75)
		...
		Debris:AddItem(PlaceToGo,.05)
		Debris:AddItem(BodyPosition,1.5)

		local HitResult,ValidEntities = HitboxModule.GetTouchingParts(Player,{ExistTime = 2, Type = "Sword", KeysLogged = NumberEvaluation, Size = Vector3.new(30,60,30), Transparency = 1, PositionCFrame = Root.CFrame},KeyData.SerializedKey,CharacterName)
		if HitResult then			
			for Index = 1, #ValidEntities do
				...
				wait(.3)
				for _ = 1,3 do
					DamageManager.DeductDamage(Character,Victim,KeyData.SerializedKey,CharacterName,{Type = ExtraData.Type, KeysLogged = NumberEvaluation})
					wait(.135)
				end	
			end
		end
	...
	["FourthAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		AnimationRemote:FireClient(Player,"Striking Tide","Play",{AdjustSpeed = .65})
		StateManager:ChangeState(Character,"Guardbroken",2.35)

		local TimeEvaluation = os.clock() - PlayerCombo.LastPressed <= .8 and true or false
		local NumberEvaluation = (PlayerCombo.Hits < 5 and TimeEvaluation and PlayerCombo.Hits + 1) or (PlayerCombo.Hits > 5 and 1) or (not PlayerCombo.TimeEvaluation and 1)

		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "TanjiroVFX", Function = "Trail", Rate = 150, Sword = Sword, TextureLength = 3, Duration = 1.5})

		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "TanjiroVFX", Function = "Striking Tide"})
		...
		for _ = 1,2 do 
			...
			coroutine.wrap(function()
				local HitResult,HitObject = HitboxModule.MagnitudeModule(Character, {Range = 7, KeysLogged = PlayerCombo.KeysLogged, Type = "Sword"}, KeyData.SerializedKey, CharacterName)
				if HitResult then
					wait(.125)
		...
		local HitObject = HitboxModule.RaycastModule(Player, {Visualize = false, Size = 18, KeysLogged = PlayerCombo.KeysLogged, Type = "Sword"}, KeyData.SerializedKey, CharacterName)
		if HitObject.Hit then
			...
			BodyVelocity.Parent = VRoot
			Debris:AddItem(BodyVelocity,.25)			
			DamageManager.DeductDamage(Character,Victim,KeyData.SerializedKey,CharacterName,{Type = ExtraData.Type})
	...
	["SecondAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "TanjiroVFX", Function = "Trail", Sword = Sword, Duration = .9})

		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "TanjiroVFX", Function = "WaterStuff", Amount = 17})
		...
		wait(.2)
		PlayerCombo.KeysLogged = NumberEvaluation
		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "TanjiroVFX", Function = "WaterWheel", Sword = Sword, Duration = .5})
		...
		coroutine.wrap(function()
			...
			wait(.125)
			local HitResult,ValidEntities = HitboxModule.GetTouchingParts(Player,{ExistTime = 2, Type = "Sword", KeysLogged = NumberEvaluation, Size = Vector3.new(8.999, 4.517, 31.284), Transparency = 1, PositionCFrame = Root.CFrame * CFrame.new(0,0,-15)},KeyData.SerializedKey,CharacterName)
			if HitResult then
				for Index = 1, #ValidEntities do
					local Victim = ValidEntities[Index]
					local VRoot, VHum = Victim:FindFirstChild("HumanoidRootPart"), Victim:FindFirstChild("Humanoid")

					NetworkStream.FireClientDistance(Character,"ClientRemote", 200, {Character = Character, Module = "TanjiroVFX", Function = "WaterWheelHit", Victim = Victim, Distance = 30})
		...
		while BodyVelocity and RunService.Stepped:Wait() do
			...
			if Root:FindFirstChild("fdfsfdsfsdfszxzzdxfzf") == nil then break end
		end
	end,
}
