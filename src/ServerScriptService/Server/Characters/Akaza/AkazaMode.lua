...
local AkazaMode = {
            ...
            if HitResult then
                ...
                if Animator then
                    local Animation = Animator:LoadAnimation(
                        ReplicatedStorage.Assets.Animations.Shared.Characters.Akaza.FiredUpVictim
                    )
                    Animation:Play()
                end
            end
    ...
    ["ThirdAbility"] = function(Player, CharacterName, KeyData, MoveData, ExtraData)
        ...
        NetworkStream.FireClientDistance(
            ...
            "ClientRemote",
            30,
            { Character = Character, Distance = 100, Module = "AkazaMode", Function = "AkazaScreen" }
        )
        NetworkStream.FireClientDistance(
            Character,
            "ClientRemote",
            200,
            { Character = Character, Distance = 100, Module = "AkazaMode", Function = "DragonTransformation" }
        )
    end,
        ...
        if HitResult then
            ...
            NetworkStream.FireClientDistance(
                ...
                200,
                {
                    ...
                    Enemy = Victim,
                    Module = "AkazaMode",
                    Function = "LightningDragonCrimsonEnd",
                }
            )
            Character.HumanoidRootPart.Anchored = true
            --// lock characters
