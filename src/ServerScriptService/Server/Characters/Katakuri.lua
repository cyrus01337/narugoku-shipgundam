...
local Katakuri = {
    ["FirstAbility"] = function(Player, CharacterName, KeyData, MoveData, ExtraData)
        ...
        if (Root.Position - Mouse.Position).Magnitude >= 35 then
            return
        end
    ...
    ["SecondAbility"] = function(Player, CharacterName, KeyData, MoveData, ExtraData)
        ...
        NetworkStream.FireClientDistance(
            ...
            "ClientRemote",
            200,
            { Character = Character, Module = "KatakuriVFX", Function = "MoguraFlow" }
        )
        for Index = 1, 7 do
        ...
        StateManager:ChangeState(Character, "Stunned", 0.2)
        SpeedManager.changeSpeed(Character, 0, 0.2, 3) --function(Character,Speed,Duration,Priority)
    end,
    ...
    ["ThirdAbility"] = function(Player, CharacterName, KeyData, MoveData, ExtraData)
        ...
        NetworkStream.FireClientDistance(
            ...
            "ClientRemote",
            1000,
            { Character = Character, Module = "KatakuriVFX", Function = "MochiRoll" }
        )
    ...
    ["ObservationHaki"] = function(Data)
        ...
        AnimationRemote:FireClient(
            Player,
            "Observation" .. DirectionList[math.random(1, #DirectionList)],
            "Play",
            { AdjustSpeed = 1 }
        )
        NetworkStream.FireClientDistance(
            ...
            "ClientRemote",
            50,
            { Character = Character, Module = "KatakuriVFX", Function = "Foresight" }
        )
    end,
}
