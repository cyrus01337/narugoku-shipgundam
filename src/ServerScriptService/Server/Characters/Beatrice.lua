...
local CreateFrameData =
    { Size = UDim2.new(1, 0, 1, 0), Position = UDim2.new(0, 0, 0, 0), Color = Color3.fromRGB(0, 0, 0), Duration = 1 }
...
local Beatrice = {
    ["FirstAbility"] = function(Player, CharacterName, KeyData, MoveData, ExtraData)
        ...
        if Data.Character == "Beatrice" then
        ...
        local Aiming = GlobalFunctions.NewInstance("BoolValue", { Parent = Character, Name = "Aiming" }, 0.3)

        StateManager:ChangeState(Character, "Attacking", 0.75)
        SpeedManager.changeSpeed(Character, 4, 2, 2.5)

        AnimationRemote:FireClient(Player, "BeatricePush", "Play")

        -- SoundManager:AddSound("Beatrice",{TimePosition = .2, Parent = Root, Volume = 10, Looped = true}, "Client", {Duration = .5})
        ...
        local HitResult, ValidEntities = HitboxModule.GetTouchingParts(
            Player,
            {
                ...
                Type = "Combat",
                KeysLogged = 1,
                Size = Vector3.new(12.565, 10.009, 30.62),
                Transparency = 1,
                PositionCFrame = Root.CFrame * CFrame.new(0, 0, -10),
            },
        ...
        if HitResult then
            for Index = 1, #ValidEntities do
                ...
                local _ = Players:GetPlayerFromCharacter(Entity)
                    and CameraRemote:FireClient(
                        Players:GetPlayerFromCharacter(Entity),
                        "CameraShake",
                        { FirstText = 8, SecondText = 10 }
                    )
            end
        end
        ...
        NetworkStream.FireClientDistance(
            ...
            "ClientRemote",
            200,
            { Character = Character, Module = "BeatriceVFX", Function = "Push" }
        )
    end,
    ...
    ["SecondAbility"] = function(Player, CharacterName, KeyData, MoveData, ExtraData)
        ...
        CameraRemote:FireClient(
            Player,
            "CameraShake",
            { FirstText = 2, SecondText = 12, Amount = 20, Time = 0.1, Type = "Loop" }
        )
        -- SoundManager:AddSound("Charge",{Parent = HumanoidRootPart, Volume = .85}, "Client")
        -- SoundManager:AddSound("beatricegain",{Parent = HumanoidRootPart, Volume = 8}, "Client")
        ...
        NetworkStream.FireClientDistance(
            ...
            "ClientRemote",
            200,
            { Character = Character, Module = "BeatriceVFX", Function = "Al~Shamac" }
        )
        ...
        TaskScheduler:AddTask(1.65, function()
            ...
            local HitResult, ValidEntities = HitboxModule.GetTouchingParts(
                Player,
                {
                    ...
                    Transparency = 1,
                    PositionCFrame = HumanoidRootPart.CFrame,
                },
                KeyData.SerializedKey,
                CharacterName
            )
            if HitResult then
                ...
                for Index = 1, #ValidEntities do
                    ...
                    NetworkStream.FireClientDistance(
                        ...
                        "ClientRemote",
                        200,
                        { Character = Character, Victim = Victim, Module = "BeatriceVFX", Function = "shamac~hit" }
                    )
                    ...
                    NetworkStream.FireClientDistance(
                        ...
                        "ClientRemote",
                        200,
                        { Character = Character, Victim = Victim, Module = "BeatriceVFX", Function = "brekapart" }
                    )
                end
            end
        end)
    ...
    ["FourthAbility"] = function(Player, CharacterName, KeyData, MoveData, ExtraData)
        ...
        local BezierBallData = AbilityData.ReturnData(Player, "FourthAbility", "Beatrice")
        ...
        for Index = 1, 3 do
            ...
            local ChosenBall = BezierBallData.ZeBall == 1 and "Color1Betrice"
                or BezierBallData.ZeBall == 2 and "Color2Betrice"
                or BezierBallData.ZeBall == 3 and "Color3Betrice"
                or "Color1Betrice"
            ...
            NetworkStream.FireClientDistance(
                ...
                200,
                {
                    ...
                    Distance = 100,
                    Module = "BeatriceVFX",
                    Function = "bezierballs",
                    WhichBall = ChosenBall,
                }
            )
