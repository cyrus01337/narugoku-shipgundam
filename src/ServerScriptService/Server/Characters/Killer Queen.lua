...
local function BreakBomb(Character,KillerQueenData,KeyData,CharacterName)
	...
	coroutine.wrap(function()
		...
		NetworkStream.FireClientDistance(Character,"ClientRemote",350,{Character = Character, Victim = workspace.World.Live[LastHitData.LastTarget], Module = "Killer QueenVFX", Function = "Switch Bomb", Distance = 100})
...
local Killer_Queen = {
	...
	["FirstAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "Killer QueenVFX", Function = "Barrage"})
	...
	["SecondAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		Debris:AddItem(BodyVelocity,.15)

		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Coin = Coin, Module = "Killer QueenVFX", Function = "Coin Flip", Distance = 100})

		local ProjectileDebounce = SetProjectileDebounce(Coin)

		local Connection; Connection = Coin.Touched:Connect(function(Hit)
			if Hit then
				if (not Hit:IsDescendantOf(Character)) and (not Hit:IsDescendantOf(workspace.World.Visuals)) and (not ProjectileDebounce.Value) then
		...
		SpeedManager.changeSpeed(Character,6,.75,4) --function(Character,Speed,Duration,Priority)
	...
	["ThirdAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		if KillerQueenData.HasBomb then
			...
			KillerQueenData.HasBomb = false
			local HitResult,HitObject = HitboxModule.MagnitudeModule(Character, {Delay = .185, Range = 7, KeysLogged = math.random(1,3)}, KeyData.SerializedKey, CharacterName)
			if HitResult then
	...
	["FourthAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "Killer QueenVFX", Function = "Traced Steps", Distance = 100})
		...
		if HitResult then
			...
			TaskScheduler:AddTask(2,function()
				NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Victim = Victim, Module = "Killer QueenVFX", Function = "Traced Steps Explode", Distance = 100})
