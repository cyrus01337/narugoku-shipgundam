...
local NatsuMode = {
		...
		if PlayerCombo.KeysLogged >= 3 then
			...
			if HitResult then
				...
				coroutine.resume(coroutine.create(function()
					wait(0.75)
					--[[ Fire Client ]]--
					NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Distance = 100, Enemy = Victim, Module = "NatsuMode", Function = "FiredUp"})
				...
				if Animator then
					local Animation = Animator:LoadAnimation(ReplicatedStorage.Assets.Animations.Shared.Characters.Natsu.FiredUpVictim)
					Animation:Play()
				end
			end
		end
	...
	["SecondAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		if HitResult then
			...
			--[[ Fire Client ]]--
			NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "NatsuMode", Function = "LightningBurst", Distance = 100, ContactPointCFrame = Root.CFrame})
			NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Distance = 100, Enemy = Victim, Module = "NatsuMode", Function = "LightningDragonHammer"})
	...
	["FourthAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		--[[ Fire Client ]]--
		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Distance = 100, Enemy = Victim, Module = "NatsuMode", Function = "LightningDragonCrimsonStart"})
		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "NatsuMode", Function = "LightningBurst", Distance = 100, ContactPointCFrame = Root.CFrame})
		wait(0.5)
		--[[ Fire Client ]]--
		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Distance = 100, Enemy = Victim, Module = "NatsuMode", Function = "LightningDragonCrimsonMove"})
		...
		if HitResult then
			...
			--[[ Fire Client ]]--
			NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "NatsuMode", Function = "LightningBurst", Distance = 100, ContactPointCFrame = Root.CFrame})
			NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Distance = 100, Enemy = Victim, Module = "NatsuMode", Function = "LightningDragonCrimsonEnd"})
			Character.HumanoidRootPart.Anchored = true
			--// lock characters
			wait(.85)
			Victim.HumanoidRootPart.Anchored = true
