...
local Natsu = {
		...
		if PlayerCombo.KeysLogged >= 3 then
			...
			if HitResult then
				...
				coroutine.wrap(function()
					wait(.5)
					CameraRemote:FireClient(Player,"CameraShake",{ FirstText = 2, SecondText = 12, Amount = 13, Time = .1, Type = "Loop"})
					wait(1.8)
				...
				NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Distance = 100, Victim = Victim, Module = "NatsuVFX", Function = "Fire Dragon's Iron Fist"})	
	...
	["SecondAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Distance = 100, Module = "NatsuVFX", Function = "CrimsonLotusStart"})
		...
		local function NoContact()
			...
			NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, ContactPoint = Direction, Distance = 100, Module = "NatsuVFX", Function = "CrimsonLotusLand"})
		end
		...
		if ray then
			...
			if partHit then
				...
				local ValidEntities = RaycastManager:GetEntitiesFromPoint(pos, workspace.World.Live:GetChildren(), {[Character] = true}, 30)
				...
				NetworkStream.FireClientDistance(Character,"ClientRemote",10,{Character = Character, Distance = 10, Module = "NatsuVFX", Function = "NatsuScreen"})

				--[[ Fire Found Landing Effects ]]--
				BodyPosition:Destroy()
				NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Distance = 100, ContactPoint = pos, Module = "NatsuVFX", Function = "CrimsonLotusLand"})
			end
		end
	...
	["ThirdAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		--[[ Fire Client ]]
		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Distance = 100, ContactPoint = Root.CFrame, Module = "NatsuVFX", Function = "PurgatoryDragonFire"})
		wait(1)

		if StateManager:Peek(Character,"Stunned") then return end

		local ValidEntities = RaycastManager:GetEntitiesFromPoint(Root.CFrame, workspace.World.Live:GetChildren(), {[Character] = true}, 30)
		for Index = 1, #ValidEntities do
			local Entity = ValidEntities[Index]
			VfxHandler.FireProc({Character = Character, Victim = Entity, Duration = 3, Damage = 1})
			DamageManager.DeductDamage(Character,Entity,KeyData.SerializedKey,CharacterName, {Type = "Combat"})
	...
	["FourthAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		--[[ Fire Animation ]]--
		AnimationRemote:FireClient(Player, "DragonRoar", "Play", {AdjustSpeed = .75})

		wait(.35)
		if StateManager:Peek(Character,"Stunned") then return end
		--[[ Fire Client ]]
		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Distance = 100, ContactPoint = Root.CFrame, Module = "NatsuVFX", Function = "FireDragonRoar"})

		wait(1.15)

		HitboxModule.GetTouchingParts(Player,{ExistTime = 2, Type = "Combat", KeysLogged = 1, Size = Vector3.new(12.565, 10.009, 53.62), Transparency = 1, PositionCFrame = Root.CFrame * CFrame.new(0,0,-25)},KeyData.SerializedKey,CharacterName)
