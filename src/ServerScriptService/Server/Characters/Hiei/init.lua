...
local Hiei = {
	["FirstAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Distance = 100, Enemy = Victim, Module = "HieiVFX", Function = "ShadowGash"})
		wait(.75)
	...
	["SecondAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		NetworkStream.FireClientDistance(Character,"ClientRemote", 200,{Character = Character, Module = "HieiVFX", Function = "SwordOfDarkness", Distance = 100, ContactPoint = Root.CFrame})
		...
		if HitResult then
			for Index = 1, #ValidEntities do
				...
				TaskScheduler:AddTask(.35,function()					
					NetworkStream.FireOneClient(Player,"ClientRemote",5,{Character = Character, Enemy = Victim, Module = "HieiVFX", Function = "ShadowHitEffect", Distance = 100})
					
					local _ = Players:GetPlayerFromCharacter(Victim) and NetworkStream.FireOneClient(Players:GetPlayerFromCharacter(Victim),"ClientRemote",5,{Character = Character, Enemy = Victim, Module = "HieiVFX", Function = "ShadowHitEffect", Distance = 100})
					local _ = Players:GetPlayerFromCharacter(Victim) and CameraRemote:FireClient(Players:GetPlayerFromCharacter(Victim),"SlashScreen")
				end)
			end
		end
	...
	["ThirdAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		SpeedManager.changeSpeed(Character,2,.8,5) --function(Character,Speed,Duration,Priority)
		StateManager:ChangeState(Character,"IFrame",.95, {IFrameType = ""})
		...
		--[[ Fire Client ]]--
		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Distance = 100, Enemy = Victim, Module = "HieiVFX", Function = "CounterStab"})
		...
		if HitResult then
			...
			NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Victim, Module = "HieiVFX", Function = "Slam", ContactPointCFrame = Victim.HumanoidRootPart.CFrame})
		end
	end,
