...
local Gilgamesh = {
	["FirstAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		if Data.Character == "Gilgamesh" then
			...
			local _ = ExtraData.Type == nil and CameraRemote:FireClient(Player, "ChangeUICooldown",{Cooldown = MoveData.Cooldown, Key = KeyData.SerializedKey, ToolName = CharacterName})
		end

		StateManager:ChangeState(Character, "Attacking", 1.5, {AllowedSkills = {["Swing"] = true}})
		local MouseHit = MouseRemote:InvokeClient(Player)

		local StartPoint = CFrame.lookAt((Character.PrimaryPart.CFrame * CFrame.new(math.random(-5, 5), math.random(3, 10), math.random(-10, -5))).Position, MouseHit.Position)

		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "GilgameshVFX", Function = "OpenPortal", StartPoint = StartPoint})
		...
		RaycastManager:CastProjectileHitbox({
			...
			Function = function(RaycastResult)
				local ValidEntities = RaycastManager:GetEntitiesFromPoint(RaycastResult.Position, workspace.World.Live:GetChildren(), {[Character] = true}, 15)
	...
	["SecondAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		local DelayBetweenProjectiles = .75

		local StoredStartPositions = {}

		for _ = 1, 7 do
			local StartPoint = CFrame.lookAt((Character.PrimaryPart.CFrame * CFrame.new(math.random(-15, 15), math.random(6, 17), math.random(-15, 0))).Position, MouseHit.Position)
			StoredStartPositions[#StoredStartPositions + 1] = StartPoint

			NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "GilgameshVFX", Function = "OpenPortal", StartPoint = StartPoint, Type = "Loop"})
		end
		...
		for Index = 1, 5 do
			...
			coroutine.wrap(function()
				wait(DelayBetweenProjectiles - math.random(.1,.3))
				RaycastManager:CastProjectileHitbox({
					Points = Points,
					Direction = Direction,
					Velocity = Velocity + 5,
	...
	["ThirdAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		if StateManager:Peek(Target,"Blocking") and DotVector >= -.5  then
		...
		if (Target.PrimaryPart.Position - Root.Position).Magnitude >= 200 then return end
		...
		for Index = 1, 4 do
			...
			RaycastManager:CastProjectileHitbox({
				...
				Function = function(RaycastResult)
					...
					if Target and Target:IsA("BasePart") and not Target:IsDescendantOf(Character) and GlobalFunctions.IsAlive(Target.Parent)then
						Next = Target.Parent
						DamageManager.DeductDamage(Character,Target.Parent,KeyData.SerializedKey,CharacterName, {Type = "Combat"})
		...
		if Hit then
			...
			local Lifetime = 10
			local DelayBetweenProjectiles = 1
			local Direction = Next and (Next:FindFirstChild("HumanoidRootPart").CFrame.Position - StartPoint.Position).Unit
			local MouseHit = MouseRemote:InvokeClient(Player)
			...
			RaycastManager:CastProjectileHitbox({
				...
				Function = function(RaycastResult)
					local Target = RaycastResult.Instance
					if Target and Target:IsA("BasePart") and not Target:IsDescendantOf(Character) and GlobalFunctions.IsAlive(Target.Parent) then
						DamageManager.DeductDamage(Character, Target.Parent ,KeyData.SerializedKey, CharacterName, {Type = "Sword"})
						NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "SwordVFX", Function = "Light", Victim = Target.Parent})
					end
				end,
				Ignore = {Character, workspace.World.Visuals}
			})
	...
	["FourthAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		if (Target.PrimaryPart.Position - Root.Position).Magnitude >= 100 then return end
		...
		for _, Point in ipairs(Points) do
			NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "GilgameshVFX", Function = "OpenPortal", StartPoint = Point, Type = "Loop"})
		end
