...
local Razor = {
	["FirstAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		if (Root.Position - Mouse.Position).Magnitude > 500 then return end
		...
		AnimationRemote:FireClient(Player,"HardThrow","Play", {AdjustSpeed = .85})
		
		NetworkStream.FireClientDistance(Character, "ClientRemote", 200, {Character = Character, Module = "RazorVFX", Function = "BallPrep"})
			
		wait(.75)
		if StateManager:Peek(Character,"Stunned") then
			NetworkStream.FireClientDistance(Character, "ClientRemote", 21431413400, {Character = Character, Module = "RazorVFX", Function = "RemoveBall"})
			return
		end
		...
		RaycastManager:CastProjectileHitbox({
			...
			Function = function(RaycastResult)
				local ValidEntities = RaycastManager:GetEntitiesFromPoint(RaycastResult.Position, workspace.World.Live:GetChildren(), {[Character] = true}, 20)
	...
	["SecondAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		if (HumanoidRootPart.Position - Mouse.Position).Magnitude > 500 then return end		
		...
		NetworkStream.FireClientDistance(Character, "ClientRemote", 200, {Character = Character, Module = "RazorVFX", Function = "VolleyKick", StartPoint = StartPoint, MouseHit = MouseHit, Lifetime = Lifetime, Velocity =  Velocity, Distance = 100,})
	...
	["ThirdAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		AnimationRemote:FireClient(Player, "ExplodingThrow", "Play", {Adjustspeed = .75})

		NetworkStream.FireClientDistance(Character, "ClientRemote", 200, {Character = Character, Module = "RazorVFX", Function = "BallPrep", Type = "Explode"})
		...
		NetworkStream.FireClientDistance(Character, "ClientRemote", 200, {Character = Character, Module = "RazorVFX", Function = "explodball", StartPoint = StartPoint, MouseHit = MouseHit, Lifetime = Lifetime, Velocity =  Velocity, Distance = 100})
		...
		RaycastManager:CastProjectileHitbox({
			...
			Visualize = false,
			Function = function(RaycastResult)
				local ValidEntities = RaycastManager:GetEntitiesFromPoint(RaycastResult.Position, workspace.World.Live:GetChildren(), {[Character] = true}, 23)
				for Index = 1, #ValidEntities do
					local Entity = ValidEntities[Index]
	...
	["FourthAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		local HitResult,ValidEntities = HitboxModule.GetTouchingParts(Player,{SecondType = "Choke", ExistTime = .35, Type = "Combat", KeysLogged = math.random(1,3), Size = Vector3.new(2.796, 4.517, 45.267), Transparency = 1, PositionCFrame = Root.CFrame * CFrame.new(0,0,-18)},KeyData.SerializedKey,CharacterName)
		if HitResult then
			for Index = 1, #ValidEntities do
				...
				NetworkStream.FireClientDistance(Character, "ClientRemote", 350, {Character = Character, Victim = Victim, Module = "RazorVFX", Function = "Chop", Distance = 100})
				
				wait()
				AnimationRemote:FireClient(Player, "KillerQueenSlap", "Play", {Delay = 0, AdjustSpeed = .85})
