...
function module.Activate(Character,Position,Spread,Target,ExtraData)
	...
	local Distance = 8
	local SPREAD = CFrame.Angles(math.rad(math.random(-Spread, Spread)), math.rad(math.random(-Spread, Spread)), 0) * CFrame.new(0, 0, -2)
	...
	for Index = 1, 1 do
		...
		coroutine.wrap(function()
			for Index = 1, 40 do
				...
				local Hit, Position, Surface = RaycastFunction(Shuriken.Position, Shuriken.CFrame * CFrame.new(0, 0, -Distance).p, Distance, workspace.World.Visuals)
				if Hit and Hit.Anchored and Hit.Transparency ~= 1 then
					if Hit.Material == Enum.Material.Wood or Hit.Material == Enum.Material.WoodPlanks then
						...
						break
					elseif Hit.Material == Enum.Material.Slate or Hit.Material == Enum.Material.Cobblestone or Hit.Material == Enum.Material.Brick or Hit.Material == Enum.Material.Pebble or Hit.Material == Enum.Material.Concrete then
						Shuriken.Stone:Play()
						Shuriken.CFrame = CFrame.new(Position, Position + Shuriken.CFrame.lookVector)
						Shuriken.Anchored = true
						break
