...
local PositionCalc = workspace.World.Enviornment.TestPart.Position.Y + .125
...
local function CreateFloorFunc(Random1, Random2, CFrame1, CFrame2, DesignatedCFrame, Duration, Size, Character, Transparency,HitList)
...
local function FloorFreeze(Area, RaycastBelow, Random1, Random2, Debounce, Duration, Size, Character, Transparency, CharacterName, KeyData, Hitlist)
	...
	if Target and (Target.Name ~= "iceFloorHitbox" or Debounce) and (PositionCalc < Position.Y or Position.Y < workspace.World.Enviornment.TestPart2.Position.Y) then
		...
		Ice.Touched:Connect(function(Hit)
			if Hit:IsDescendantOf(workspace.World.Live) and Hit.Parent:FindFirstChild("Humanoid") and not Hit:IsDescendantOf(Character) and not Hit.Parent:FindFirstChild("floorFreezehit") and not StateManager:Peek(Hit.Parent,"Blocking") and not table.find(Hitlist,Hit.Parent) then					
				...
				coroutine.wrap(function()
					...
					NetworkStream.FireClientDistance(Character,"ClientRemote",150,{Victim = Hit.Parent, Module = "AokijiVFX", Function = "Ice FreezeBreak"})
				end)()
			end
		end)
...
local Aokiji = {
	["FirstAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)		
		...
		if (Root.Position - Mouse.Position).Magnitude >= 40 then return end				
		...
		SpeedManager.changeSpeed(Character,4,.5,3,false) --function(Character,Speed,Duration,Priority)
		...
		GlobalFunctions.NewInstance("BoolValue",{Parent = Character, Name = "Aiming"},.35)
		...
		RaycastManager:CastProjectileHitbox({
			...
			Function = function(RaycastResult)
				local ValidEntities = RaycastManager:GetEntitiesFromPoint(RaycastResult.Position, workspace.World.Live:GetChildren(), {[Character] = true}, 7.5)
	...
	["SecondAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		NetworkStream.FireClientDistance(Character,"ClientRemote",150,{Character = Character, Module = "AokijiVFX", Function = "Ice Floor"})
	...
	--	NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "TanjiroVFX", Function = "WaterWheel", Debounce = not StateManager:Peek(Character,"InAir") and true and false})
		...
		TaskScheduler:AddTask(.35,function()
			...
			NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "AokijiVFX", Function = "Ice Sword", Index = 100})
			HitboxModule.MagnitudeModule(Character, {Range = 10, KeysLogged = 1, Type = "Sword"}, KeyData.SerializedKey, CharacterName)

			wait(.35)
			for Index = 1,2 do
				HitboxModule.MagnitudeModule(Character, {Delay = .185, Range = 7.5, KeysLogged = 1, Type = "Sword"}, KeyData.SerializedKey, CharacterName)
				-- SoundManager:AddSound("SwordSwing",{Parent = HumanoidRootPart, Volume = 5},"Client")
				NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "AokijiVFX", Function = "Ice Sword", Index = Index})
				wait(.25)
			end
			wait(.35)
			AokijiData.Guardbreak = true
			NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "AokijiVFX", Function = "Ice Sword", Index = 3})
			HitboxModule.MagnitudeModule(Character, {Delay = .1, Range = 7.5, KeysLogged = 1, Type = "Sword"}, KeyData.SerializedKey, CharacterName)
		...
		while BodyVelocity and RunService.Stepped:Wait() do
			...
			if HumanoidRootPart:FindFirstChild("fdfsfdsfsdfszxzzdxfzf") == nil then break end
		end
	end,
	...
	["ThirdAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		NetworkStream.FireClientDistance(Character,"ClientRemote",150,{Character = Character, Module = "AokijiVFX", Function = "Ice Freeze"})
		
		for Index = 1,18 do
			FloorFreeze(HumanoidRootPart.CFrame * CFrame.new(0,1,-Index * 3.8), 100, 10, 15, nil, 1.75, Vector3.new(10, 0.989, 10), Character,1,CharacterName,KeyData,AokijiData)
			wait()
		end
	end,
	...
	["FourthAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		StateManager:ChangeState(Character,"Attacking",.385)
		...
		local HitResult,ValidEntities = HitboxModule.GetTouchingParts(Player,{ExistTime = 2, Type = "Combat",Size = Vector3.new(25.147, 18.374, 42.655), Transparency = 1, PositionCFrame = Root.CFrame * CFrame.new(0,0,-25)},KeyData.SerializedKey,CharacterName)
		if HitResult then
			for Index = 1, #ValidEntities do
				local Entity = ValidEntities[Index]
		...
		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "AokijiVFX", Function = "Ice Stomp"})
	end,
}
