...
local Sasuke = {
    ["FirstAbility"] = function(Player, CharacterName, KeyData, MoveData, ExtraData)
        ...
        StateManager:ChangeState(Character, "Attacking", Sound.TimeLength + 0.35)
            ...
            RaycastManager:CastProjectileHitbox({
                ...
                Function = function(RaycastResult)
                    ...
                    for Index = 1, #ValidEntities do
                        ...
                        VfxHandler.FireProc({ Character = Character, Victim = Entity, Damage = 0.5, Duration = 5 })
    ...
    ["SecondAbility"] = function(Player, CharacterName, KeyData, MoveData, ExtraData)
        ...
        CameraRemote:FireClient(Player, "CreateBlur", { Size = 25, Length = 0.5 })
        Root.Anchored = true
        Humanoid.WalkSpeed = 30

        StateManager:ChangeState(Character, "IFrame", 0.5, { IFrameType = "" })
        ...
        Connection = Clone.Touched:Connect(function(Hit)
            if not Hit:IsDescendantOf(Character) and Hit:IsDescendantOf(workspace.World.Live) then
                if Hit.Parent:FindFirstChild("Humanoid") and Hit.Parent.Humanoid.Health > 0 then
                    ...
                    StateManager:ChangeState(Character, "IFrame", 0.8, { IFrameType = "" })
        ...
        ValueConnection = ChidoriValue:GetPropertyChangedSignal("Value"):Connect(function()
        ...
        while os.clock() - StartTime <= 3.35 and not Character:FindFirstChild("BreakChidoriClient") do
            if Humanoid.MoveDirection == Vector3.new() then
                ...
                SpeedManager.changeSpeed(Character, 14, 0.35, 6e6) --function(Character,Speed,Duration,Priority)
            else
                ChidoriValue.Value = 2
                SpeedManager.changeSpeed(Character, 32, 0.35, 6e6) --function(Character,Speed,Duration,Priority)
            end
    ...
    ["ThirdAbility"] = function(Player, CharacterName, KeyData, MoveData, ExtraData, Modules)
        ...
        local SasukeData = AbilityData.ReturnData(Player, "ThirdAbility", "Sasuke")

        if SasukeData.StoredSkill == "" then
            ...
            if
                SasukeData.CopiedSkillData
                and SasukeData.CopiedSkillData.Copyable
                and SasukeData.CopiedSkillData.Copyable == true
            then
            ...
            local CopiedData = AbilityData.ReturnData(Player, SasukeData.StoredSkill, SasukeData.SharinganContact)
            ...
            CacheModule(Player, SasukeData.SharinganContact, {
                ...
                KeyName = SasukeData.StoredSkill == "FirstAbility" and "Z"
                    or SasukeData.StoredSkill == "SecondAbility" and "X"
                    or SasukeData.StoredSkill == "ThirdAbility" and "C"
                    or "V",
            }, SkillData, ExtraData)

            SasukeData.StoredSkill = ""
            SasukeData.SharinganContact = ""
    ...
    ["FourthAbility"] = function(Player, CharacterName, KeyData, MoveData, ExtraData)
        ...
        if
            not Humanoid:GetState() == Enum.HumanoidStateType.Landed
        ...
        NetworkStream.FireClientDistance(
            ...
            "ClientRemote",
            200,
            { Character = Character, Module = "SasukeVFX", Function = "Shuriken" }
        )
        ...
        local _ = SasukeData.Falling
            and GlobalFunctions.NewInstance(
                "BodyVelocity",
                {
                    Parent = Root,
                    MaxForce = Vector3.new(100000000, 100000000, 100000000),
        ...
        for Index = 1, 3 do
            ...
            if ExtraData.Victim == nil then
                ExtraData.Victim = nil
            end
            if ExtraData.Victim then
                SasukeData.Spread = 2.6
            end

            ShurikenThrow.Activate(
                Character,
                ExtraData.MouseHit,
                SasukeData.Spread,
                ExtraData.Victim,
                { KeyData = KeyData, CharacterName = CharacterName }
            )
        end
    end,
}
