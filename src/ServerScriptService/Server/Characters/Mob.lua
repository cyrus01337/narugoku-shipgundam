...
local Mob = {
		...
		if PlayerCombo.KeysLogged >= 3 then
			...
			if HitResult then
				...
				coroutine.wrap(function()
					wait(.5)
					NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Victim = Victim, Module = "MobVFX", Function = "ChokeHoldTehe?"})
					CameraRemote:FireClient(Player, "ColorCorrection",{ Type = "Create", Name = "Gravity", TimeBeforeRemove = 1.25, Length = .75, TweenColor = Color3.fromRGB(161,83,255)})
					if Players:GetPlayerFromCharacter(Victim) then
						CameraRemote:FireClient(Players:GetPlayerFromCharacter(Victim), "ColorCorrection",{ Type = "Create", Name = "Gravity", TimeBeforeRemove = 1.25, Length = .75, TweenColor = Color3.fromRGB(161,83,255)})
					end
					CameraRemote:FireClient(Player,"CameraShake",{ FirstText = 2, SecondText = 12, Amount = 15, Time = .1, Type = "Loop"})
					wait(.5)
					CameraRemote:FireClient(Player,"CreateBlur",{Size = 10, Length = .225})
				...
				if Animator then
					local Animation = Animator:LoadAnimation(ReplicatedStorage.Assets.Animations.Shared.Characters.Mob.MobFlyVictim)
					Animation:Play()
				end
				NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Distance = 100, Victim = Victim, Module = "MobVFX", Function = "PhyscoChoke"})

				wait(.65)
				for _ = 1,16 do
					StateManager:ChangeState(Character, "IFrame", .5)
					DamageManager.DeductDamage(Character,Victim,KeyData.SerializedKey,CharacterName,{SecondType = "Choke", Type = "Combat", KeysLogged = 3})
					wait(.1175)
				end
	...
	["SecondAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		local HitResult,ValidEntities = HitboxModule.GetTouchingParts(Player,{SecondType = "Choke", ExistTime = .35, Type = "Combat", KeysLogged = NumberEvaluation, Size = Vector3.new(8.999, 4.517, 17.267), Transparency = 1, PositionCFrame = Root.CFrame * CFrame.new(0,0,-2)},KeyData.SerializedKey,CharacterName)
		if HitResult then		
			for Index = 1, #ValidEntities do
				...
				local Anim = VHum:LoadAnimation(ReplicatedStorage.Assets.Animations.Shared.Characters.Mob.PhyscoSlamVictim)
				...
				NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Distance = 100, Victim = Victim, Module = "MobVFX", Function = "PhyscoSlamUp"})
				...
				local Position = RaycastResult and RaycastResult.Position + Vector3.new(0,18,0) or VRoot.Position + Vector3.new(0,18,0)

				local Tween = TweenService:Create(VRoot,TweenInfo.new(.5,Enum.EasingStyle.Quad),{Position = Position})
				Tween:Play()
				Tween:Destroy()
				...
				if StateManager:Peek(Character,"Stunned") then 
					NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Distance = 100, Victim = Victim, Module = "MobVFX", Function = "CancelSlam"})

					local Tween = TweenService:Create(VRoot,TweenInfo.new(.215,Enum.EasingStyle.Quad),{Position = RaycastResult.Position})
					Tween:Play()
					Tween:Destroy()
					return 
				end
				...
				NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Distance = 100, Victim = Victim, Module = "MobVFX", Function = "PhyscoSlamDown"})
	...
	["ThirdAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		for Index = 1, RockAmount - 1 do
			local RandomPosition = Character.PrimaryPart.Position + Vector3.new(math.random(-20,20),math.random(10,20),math.random(-20,20))
		...
		for Index = 1, #Rocks do
			...
			RaycastManager:CastProjectileHitbox({
				...
				Function = function(RaycastResult) 
					local ValidEntities = RaycastManager:GetEntitiesFromPoint(RaycastResult.Position, workspace.World.Live:GetChildren(), {[Character] = true}, ExplosionRange)
					for Index = 1, #ValidEntities do
	...
	["FourthAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		RaycastManager:CastProjectileHitbox({
			...
			Visualize = false, 
			Function = function(RaycastResult) 
				local ValidEntities = RaycastManager:GetEntitiesFromPoint(RaycastResult.Position, workspace.World.Live:GetChildren(), {[Character] = true}, 12)
				for Index = 1, #ValidEntities do
					local Entity = ValidEntities[Index]
