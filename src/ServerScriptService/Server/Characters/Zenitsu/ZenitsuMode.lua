...
local ZenitsuMode = {
    ...
    ["Transformation"] = function(Player, CharacterName, KeyData, MoveData, ExtraData)
        ...
        NetworkStream.FireOneClient(
            ...
            "ClientRemote",
            5,
            { Character = Character, Module = "AokijiMode", Function = "Cutscene" }
        )
    ...
    ["FirstAbility"] = function(Player, CharacterName, KeyData, MoveData, ExtraData)
        ...
        local CreateFrameData = {
            Size = UDim2.new(1, 0, 1, 0),
            Position = UDim2.new(0, 0, 0, 0),
            Color = Color3.fromRGB(255, 230, 101),
            Duration = 0.1,
        }
        CameraRemote:FireClient(Player, "CreateFlashUI", CreateFrameData)
        NetworkStream.FireClientDistance(
            ...
            "ClientRemote",
            200,
            { Character = Character, Module = "ZenitsuMode", Function = "TransformationOld" }
        )
        TaskScheduler:AddTask(0.85, function()
            local CreateFrameData = {
                Size = UDim2.new(1, 0, 1, 0),
                Position = UDim2.new(0, 0, 0, 0),
                Color = Color3.fromRGB(255, 230, 101),
            ...
            HitboxModule.GetTouchingParts(
                Player,
                {
                    ...
                    Size = Vector3.new(20.417, 4.517, 58.929),
                    Transparency = 1,
            ...
            NetworkStream.FireClientDistance(
                ...
                "ClientRemote",
                200,
                { Character = Character, Module = "ZenitsuMode", Function = "gitredy" }
            )
            ...
            for Index = 1, 6 do
                ...
                Beam.CFrame = CFrame.new(Root.CFrame.Position, CFrame.new(position).Position)
                    * CFrame.new(0, 0, -EndMag / 2)
                ...
                NetworkStream.FireClientDistance(
                    ...
                    200,
                    {
                        Character = Character,
                        Module = "ZenitsuMode",
                        Function = "SixFoldTeleport",
                        Configurations = {
                            Start = Start,
                            End = End,
                            NumberOfParts = math.clamp((Start - End).Magnitude / 15, 3, 20),
                        },
                    }
                )
            ...
            if RaycastResult and RaycastResult.Instance and (RaycastResult.Position - point).Magnitude < 100 then
                ...
                NetworkStream.FireClientDistance(
                    ...
                    200,
                    {
                        ...
                        Function = "Menbere",
                        Distance = 100,
                        ContactPointCFrame = Root.CFrame * CFrame.new(0, 0, 36),
                    }
                )
            end
        end)
    ...
    ["SecondAbility"] = function(Player, CharacterName, KeyData, MoveData, ExtraData)
        ...
        NetworkStream.FireClientDistance(
            ...
            "ClientRemote",
            200,
            { Character = Character, Module = "ZenitsuMode", Function = "WhirlWindRelease", Part = Part }
        )
        ...
        for Index = 1, 9 do
            ...
            VRoot.CFrame = CFrame.new(
                Base
                    + Vector3.new(
                        math.random(15, 25) * RandomCalculation,
                        Yincrement,
                        math.random(15, 25) * RandomCalculation
                    )
            )
            --
            local Beam = ReplicatedStorage.Assets.Effects.Meshes.Whirldwindball:Clone()
            Beam.BrickColor = math.random(1, 2) == 1 and BrickColor.new("Pastel blue-green")
            ...
            Beam.Size = Vector3.new(End, 5, 5)
            Beam.CFrame = CFrame.new(Part.CFrame.Position, VRoot.CFrame.Position)
                * CFrame.new(0, 0, -End / 2)
                * CFrame.fromEulerAnglesXYZ(0, math.rad(90), 0)
            Beam.Parent = workspace.World.Visuals
