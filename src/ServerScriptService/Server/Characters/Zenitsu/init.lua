...
local Zenitsu = {
	["FirstAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "ZenitsuVFX", Function = "ThunderClapandFlash"})
		TaskScheduler:AddTask(.725,function()
			HitboxModule.GetTouchingParts(Player,{Delay = .935, ExistTime = 2, Type = "Sword", KeysLogged = math.random(1,3), Size = Vector3.new(12.417, 4.517, 58.929), Transparency = 1, PositionCFrame = Root.CFrame * CFrame.new(0,0,-18.5)},KeyData.SerializedKey,CharacterName)	
			NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "ZenitsuVFX", Function = "Menbere", Distance = 100, ContactPointCFrame = Root.CFrame})
		end)
	...
	["SecondAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		delay(.2,function() CameraRemote:FireClient(Player,"TweenObject",{LifeTime = .35,EasingStyle = Enum.EasingStyle.Linear,EasingDirection = Enum.EasingDirection.Out, Return = true,Properties = {FieldOfView = 100}})	end)		
		TaskScheduler:AddTask(1,function()
			AnimationRemote:FireClient(Player,"Sleeping")
			
			NetworkStream.FireOneClient(Player,"ClientRemote",1,{Character = Character, Module = "ZenitsuVFX", Function = "SleepChangeCameraTorso"})
			NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "ZenitsuVFX", Function = "Sleeping"})
			AnimationRemote:FireClient(Player,"Sleeping","Play",{FadeTime = .2, Looped = true})
			...
			while true do
				...
				if Player:FindFirstChild("ZenitsuBar") then
					if Player.ZenitsuBar.Value <= 295 then
						Player.ZenitsuBar.Value += .55
					end
				end
				CameraRemote:FireClient(Player,"CustomBarToValue",{Character = "Zenitsu", WhichValue = "ZenitsuBar"})
				if ModeNumber.Value <= ModeData.MaxModeValue then
					ModeNumber.Value += .2
				end
				wait()
				--if Player.ZenitsuBar.Value >= 295 then break end
				if SkillData.Skill == "Swing" or StateManager:Peek(Character,"Stunned") then break end
				RunService.Heartbeat:Wait()
			end
			local ZenitsuData = AbilityData.ReturnData(Player,"SecondAbility","Zenitsu")
			ZenitsuData.Sleeping = false
			
			NetworkStream.FireOneClient(Player,"ClientRemote",1,{Character = Character, Module = "ZenitsuVFX", Function = "SleepRevertCamera"})
			...
			coroutine.wrap(function()
				while Player:FindFirstChild("ZenitsuBar") do
					...
					CameraRemote:FireClient(Player,"CustomBarToValue",{Character = "Zenitsu", WhichValue = "ZenitsuBar"})
					wait(2)
					if ZenitsuData.Sleeping then break end
					if Player.ZenitsuBar.Value <= 0 then break end
				end
				CameraRemote:FireClient(Player,"CustomBarToValue",{Character = "Zenitsu", WhichValue = "ZenitsuBar"})
			end)()		
			...
			SpeedManager.changeSpeed(Character,14,.15,4e4)
	...
	["ThirdAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		wait(.2)
		NetworkStream.FireClientDistance(Character,"ClientRemote",200,{Character = Character, Module = "ZenitsuVFX", Function = "Distance Thunder"})		
		CameraRemote:FireClient(Player,"CameraShake",{ FirstText = 6, SecondText = 12, Amount = 2, Time = .1, Type = "Loop"})

		local CreateFrameData = {Size = UDim2.new(1,0,1,0); Position = UDim2.new(0,0,0,0); Color = Color3.fromRGB(202, 195, 97); Duration = .75}
		CameraRemote:FireClient(Player,"CreateFlashUI",CreateFrameData)
		...
		NetworkStream.FireClientDistance(Character, "ClientRemote", 200, {Character = Character, Module = "RazorVFX", Function = "HardThrow", StartPoint = StartPoint, MouseHit = MouseHit, Lifetime = Lifetime, Velocity =  Velocity, Distance = 100, Direction = Direction, Ponts = Points,})
		...
		RaycastManager:CastProjectileHitbox({
			...
			Function = function(RaycastResult)
				local ValidEntities = RaycastManager:GetEntitiesFromPoint(RaycastResult.Position, workspace.World.Live:GetChildren(), {[Character] = true}, 10)
				for Index = 1, #ValidEntities do
					local Entity = ValidEntities[Index]
					DamageManager.DeductDamage(Character,Entity,KeyData.SerializedKey,CharacterName, {Type = "Sword"})
	...
	["FourthAbility"] = function(Player,CharacterName,KeyData,MoveData,ExtraData)
		...
		StateManager:ChangeState(Character,"Attacking",.3)
		...
		TaskScheduler:AddTask(.5,function()
			...
			CameraRemote:FireClient(Player,"CameraShake",{ FirstText = 8, SecondText = 12, Amount = 2, Time = .1, Type = "Loop"})			
			-- SoundManager:AddSound("UnSheath",{Parent = Root, Volume = 1},"Client")
