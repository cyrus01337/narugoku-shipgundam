...
local Include =
    { "Accessory", "Clothing", "CharacterMesh", "BodyColors", "ShirtGraphic", "MeshPart", "Hat", "Hats", "FakeHead" }
...
local function SetHats(Character, Morph)
    if Morph:FindFirstChild("Hats") == nil then
        return
    end
...
function AppearanceManager.Morph(Character, Morph)
    ...
    if Character == nil or SelectedMorph == nil then
        return
    end
    ...
    if Character:FindFirstChild("HatFolder") then
        Character:FindFirstChild("HatFolder"):Destroy()
    end
