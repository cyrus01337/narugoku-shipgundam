...
local function CreateRock(Result)
    ...
    local Tween =
        TweenService:Create(Rock, TweenInfo.new(0.15, Enum.EasingStyle.Linear), { Position = Result.Position })
    Tween:Play()
    Tween:Destroy()
...
local ZenitsuVFX = {
    ...
    ["Zap"] = function(Data)
        ...
        for _ = 1, 5 do
            ...
            createhugelightning(
                Root.Position,
                Data.Position,
                math.clamp(math.floor((Root.Position - Data.Position).Magnitude / 15), 3, 999),
    ...
    ["Menbere"] = function(Data)
        ...
        DahNoob.CFrame = CFrame.lookAt(StartPoint.Position, StartPoint.Position + Dir)
            * CFrame.Angles(0, math.rad(90), 0)
        DahNoob.Parent = workspace.World.Visuals

        local StartTween = TweenService:Create(
            DahNoob,
            TweenInfo.new(0.85, Enum.EasingStyle.Sine, Enum.EasingDirection.Out, 0, false, 0.135),
    ...
    ["ThunderClapandFlash"] = function(Data)
        ...
        if Character.Name == Players.LocalPlayer.Name then
            GlobalFunctions.FreshShake(20, 25, 0.5, 0.2, 0)
        end
        ...
        for Index = 1, 8 do
            ...
            bolt.PulseSpeed = 2
            bolt.PulseLength = 0.75
            bolt.FadeLength = 0.55
            if Index % 2 == 0 then
