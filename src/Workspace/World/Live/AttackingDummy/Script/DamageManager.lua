...
function DamageManager.DeductDamage(Character,Victim,SkillName, CharacterName, KeysLogged, ExtraData)
	...
	local ModifierCopy = AbilityData.ReturnData(Character,SkillName,IndexCalculation) or CombatData.ReturnData(Character,SkillName) 
	...
	if VictimHumanoid.Health >= 1 and Humanoid.Health >= 0 and Victim ~= Character then
		...
		elseif StateManager:Peek(Victim,"IFrame") then
			NetworkStream.FireClientDistance(Character,"ClientRemote",50,{Character = Character, Damage = 5 , Victim = Victim, Module = "PlayerClient", Function = "DamageIndication", StunTime = StunTime})
