...
local function CreateRock(Result)
	...
	TaskScheduler:AddTask(1, function()
		local Tween = TweenService:Create(Rock, TweenInfo.new(0.5, Enum.EasingStyle.Linear), {Position = Result.Position - Vector3.new(0,Rock.Size.Y,0), Orientation = Vector3.new(math.random(-360,360),math.random(-360,360),math.random(-360,360)), Size = Vector3.new(0,0,0)})
		Tween:Play()
		Tween:Destroy()
	end)
end
...
local CombatVFX = {	
	...
	StartBarrage = function(Data)
		...
		while true do
			...
			if os.clock() - StartTime >= 1.15 then break end
		end
		Dust.Attachment.dust1.Enabled = false
	...
	LastBarrageHit = function(Data)
		...
		VfxHandler.Shockwave2(Root.CFrame * CFrame.new(0,0,-3) * CFrame.Angles(math.rad(90), 0, 0), Root.CFrame * CFrame.new(0,0,-3) *  CFrame.Angles(math.rad(90), 0, 0), BrickColor.new("White"), Vector3.new(8, 1, 8), Vector3.new(0, 1, 0), .25, 0, Enum.EasingStyle.Quad, Color3.fromRGB(255, 255, 255), Enum.Material.Neon, "Rings");
	...
	Block = function(Data)
		...
		BlockAnimation = Animator:LoadAnimation(ReplicatedStorage.Assets.Animations.Shared.Combat.Block[WeaponType.."BlockHitReaction"])
		BlockAnimation:Play()
		end
	...
	Light = function(Data)
		...
		if Data.SecondType == "Choke" then return end
		
		local VHumanoid,VRoot = Victim:FindFirstChild("Humanoid"), Victim:FindFirstChild("HumanoidRootPart")	
		local Animator = VHumanoid:FindFirstChildOfClass("Animator")
		
		if VRoot:FindFirstChild("SnakeAwakenKnockback") == nil then VfxHandler.Orbies({Parent = VRoot, Speed = .35, Cframe = CFrame.new(0,0,3), Amount = 6, Circle = true}) end
		local PunchedParticle = ReplicatedStorage.Assets.Effects.Particles.ParticleAttatchments.Punched:Clone()
		PunchedParticle.Parent = VRoot
		...
		if Data.SecondType =="SlamDown" then return end
		
		if Victim and Animator and Humanoid then
			local Animation = Animator:LoadAnimation(ReplicatedStorage.Assets.Animations.Shared.Combat.HitReaction["HitReaction"..(KeysLogged == 0 and 1 or KeysLogged)]) --KeysLogged == 0 and 1 or KeysLogged
			Animation:Play()
		end
	end,
	...
	LastSwing = function(Data)
		...
		Debris:AddItem(Trail, .65)

		local Ting = Character["Right Arm"]:Clone()
		Ting.Material = "Neon"
		Ting.Transparency = .125
		Ting.Color = Color3.fromRGB(157, 255, 241)
		Ting.Size = Character["Right Arm"].Size + Vector3.new(.1,.1,.1)
		...
		local Tween = TweenService:Create(Ting,TweenInfo.new(.125,Enum.EasingStyle.Cubic,Enum.EasingDirection.In,0 ,false),{["Transparency"] = 1})
		Tween:Play()
		Tween:Destroy()
	end,
	...
	AerialKnockBackEffect = function(Data)
		...
		if RaycastResult and RaycastResult.Instance and (RaycastResult.Position - Start).Magnitude <= 300 then
			...
			VfxHandler.Orbies({Parent = VRoot, Speed = .5, Size = Vector3.new(1, 1, 12), Cframe = CFrame.new(0,0,15), Amount = 10, Sphere = true})
			...
			local partHit,pos = workspace:FindPartOnRayWithIgnoreList(ray, {Character, workspace.World.Visuals, Victim}, false, false)
			if partHit then
