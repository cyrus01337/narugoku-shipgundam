...
local function AdjustableToSet(Part, Number)
    ...
    local Tween = TweenService:Create(
        ...
        { ["CFrame"] = Part.CFrame * CFrame.new(0, 0, -Number / 2), ["Size"] = SizeIndex, ["Transparency"] = 1 }
    )
    Tween:Play()
    Tween:Destroy()
...
local function ToBurn(Part, Random1, Random2, CFrameParameter, LastRandom)
    if Part then
        ...
        local Tween = TweenService:Create(
            ...
            {
                ["WorldPosition"] = (Part.CFrame * CFrame.new(0, 0, CFrameParameter)).p + Vector3.new(
                    math.random(-LastRandom, LastRandom),
                    math.random(-LastRandom, LastRandom),
                    math.random(-LastRandom, LastRandom)
                ),
            }
...
local SanjiModeVFX = {
    ...
    ["FLASHKICKHITTTTT"] = function(PathData)
        ...
        local _ = Character.Name == Players.LocalPlayer.Name and GlobalFunctions.FreshShake(185, 35, 1, 0.15)
        ...
        EnemyVelocity.Velocity = CFrame.new(
        ...
        Debris:AddItem(EnemyVelocity, 0.1)
    ...
    ["DemonAxeExplode"] = function(PathData)
        ...
        local RaycastResult = workspace:Raycast(VRoot.Position, Vector3.new(0, -1000, 0), RayParam)
            or {
                Position = VRoot.Position + Vector2.yAxis * -1000,
            }
        ...
        if Target then
            ...
            Dust.Attachment.BurningPart.Size = NumberSequence.new({
            ...
            local Tween = TweenService:Create(
                PartTing.Mesh,
                TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                { ["Scale"] = Vector3.new(0, PartTing.Mesh.Scale.Y + 15, 0), ["Offset"] = Vector3.new(0, 22.5, 0) }
            )
            Tween:Play()
            Tween:Destroy()

            Debris:AddItem(PartTing, 0.15)
            ...
            for Index = 1, 8 do
                ...
                PartTingz.CFrame = CFrame.new(Position)
    ...
    ["AscendKick"] = function(PathData)
        ...
        local Tween = TweenService:Create(
            ...
            { ["Offset"] = Vector3.new(0, 47.5, 0), ["Scale"] = Vector3.new(0, SphereEffect.Mesh.Scale.Y + 15, 0) }
        )
        Tween:Play()
        Tween:Destroy()
        ...
        VfxHandler.Shockwave2(
            CFrameCalculation * CFrame.new(0, 60, 0),
            CFrameCalculation,
        ...
        VfxHandler.Shockwave2(
            CFrameCalculation * CFrame.new(0, 20, 0),
            CFrameCalculation,
        ...
        local Tween = TweenService:Create(
            Part.Mesh,
            TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
            { ["Offset"] = Vector3.new(0, 25, 0), ["Scale"] = Vector3.new(0, 0, 0) }
        )
        Tween:Play()
        Tween:Destroy()
        ...
        local Tween =
            TweenService:Create(MeshPart, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                ["CFrame"] = MeshPart.CFrame
                    * CFrame.new(50, 0, 0)
                    * CFrame.Angles(math.rad(math.random(-360, 360)), 0, 0),
        ...
        for _ = 1, 20 do
            ...
            local Tween = TweenService:Create(
                Sph.Mesh,
                TweenInfo.new(RandomIndex2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                {
    ...
    ["TeleportKick"] = function(PathData)
        ...
        local Tween = TweenService:Create(
            Root,
            TweenInfo.new(PathData.Duration or 0.15, Enum.EasingStyle.Quad),
            { CFrame = VRoot.CFrame * CFrame.new(0, 0, 3) }
        )
        Tween:Play()
        Tween:Destroy()
    end,
    ...
    ["AscendTrail"] = function(PathData)
        ...
        local _ = Character.Name == Players.LocalPlayer.Name and GlobalFunctions.FreshShake(180, 100, 1, 0.15)
        ...
        TrailPart.CFrame = CFrame.new(Root.Position, Root.CFrame.Position)
            * CFrame.new(0, 0, -MagnitudeIndex / 2)
            * CFrame.Angles(math.rad(90), 0, 0)
        ...
        local Tween = TweenService:Create(
            TrailPart.Mesh,
            TweenInfo.new(0.25, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out),
            { ["Offset"] = Vector3.new(0, -MagnitudeIndex / 2, 0), ["Scale"] = Vector3.new(0, 0, 0) }
        )
        Tween:Play()
        Tween:Destroy()
        ...
        for Index = 1, IndexCalculation do
            ...
            raycastParam.FilterDescendantsInstances = { workspace.World.Visuals, IndexCalculation }
            raycastParam.FilterType = Enum.RaycastFilterType.Exclude

            local RaycastResult = workspace:Raycast(LerpIndex.p, LerpIndex.upVector * -10, raycastParam)
            local Target, Position = RaycastResult.Instance, RaycastResult.Position
            ...
            local Tween = TweenService:Create(
                Trail.PointLight,
                TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                { ["Color"] = BrickColor.new("Gold").Color, ["Range"] = 0 }
            )
            Tween:Play()
            Tween:Destroy()
    ...
    ["Anti_Manner_Kick_Course"] = function(PathData)
        ...
        local Tween = TweenService:Create(
            ...
            {
                ["CFrame"] = Twister.CFrame * CFrame.Angles(0, math.rad(180), 0) * CFrame.new(0, -15, 0),
                ["Transparency"] = 1,
            }
        )
        Tween:Play()
        ...
        for Index = 1, 10 do
            ...
            local Tween = TweenService:Create(
                ...
                { ["CFrame"] = Speedline.CFrame * CFrame.new(0, math.random(10, 80), 0), ["Transparency"] = 1 }
            )
            Tween:Play()
            Tween:Destroy()

            Debris:AddItem(Speedline, 0.5)
        end
        ...
        VfxHandler.Rockszsz({
            ...
            FirstDuration = 0.5, -- Rock tween outward start duration
    ...
    ["PartyTable"] = function(PathData)
        ...
        while true do
            ...
            ToPart(
                End * CFrame.new(2, math.random(-10, 10) / 10 + 2, 0),
                End,
                BrickColorIndex,
                Vector3.new(Rot.X, 0, Rot.z),
                Rot / 4,
            ...
            if os.clock() - StartTime >= 1.725 then
                break
            end
        end
        Dust.Attachment.dust1.Enabled = false
    ...
    ["Spectre"] = function(PathData)
        ...
        while true do
            ...
            SpectreModel:SetPrimaryPartCFrame(
                Root.CFrame
                    * CFrame.Angles(
            ...
            RingEffect.CFrame = SpectreModel.PrimaryPart.CFrame
                * CFrame.new(0, 0, -8)
                * CFrame.Angles(math.rad(90), math.random(-360, 360), 0)
            ...
            local Tween = TweenService:Create(
                RingEffect,
                TweenInfo.new(0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.Out),
                { ["Size"] = Vector3.new(RandomCalculation, RingEffect.Size.Y, RandomCalculation), ["Transparency"] = 1 }
            )
            Tween:Play()
            Tween:Destroy()
            ...
            VfxHandler.Orbies({
                Parent = RingEffect,
                Speed = 0.2,
                Cframe = CFrame.new(0, 0, math.random(3, 5)),
                Amount = 1,
                Circle = true,
            })
