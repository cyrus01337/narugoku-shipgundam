...
local AokijiMode = {
    ...
    ["FreezingGrab"] = function(Data)
        ...
        coroutine.resume(coroutine.create(function()
            while wait() do
                ...
                if os.clock() - StartTime >= 1.75 then
                    break
                end
            end
        end))
        ...
        VfxHandler.FakeBodyPart({
            ...
            Object = "Left Leg",
            Material = "Neon",
            Color = Color3.fromRGB(79, 113, 136),
            TweenColor = Color3.fromRGB(136, 195, 234),
            Transparency = 1,
            Duration = 1.5,
            Delay = 0.1,
        })
    ...
    ["Transformation"] = function(Data)
        ...
        coroutine.resume(coroutine.create(function()
            for Index = 1, 2 do
                ...
                Debris:AddItem(Ball, 0.75)
                wait(0.2)
            end
        end))

        VfxHandler.FloorFreeze(
            Root.CFrame,
            100,
            10,
            15,
    ...
    ["Ice Bird"] = function(PathData)
        ...
        Fireball.Bird.CFrame = CFrame.lookAt(StartPoint.Position, StartPoint.Position + Dir)
        ...
        GlobalFunctions.FreshShake(45, 15, 0.5, 0.2, 0)
        ...
        local Connection
        Connection = Fireball.Bird.Touched:Connect(function(Hit)
            if
                ...
                and not Hit:IsDescendantOf(workspace.World.Enviornment)
            then
                icebirdHit = true
                Explosions.Spear({
                    Character = Character,
                    RaycastResult = Fireball.Bird,
                    Spear = Fireball.Bird,
        ...
        coroutine.wrap(function()
            while not icebirdHit do
                VfxHandler.FloorFreeze(
                    Fireball.Bird.CFrame,
                    100,
                    10,
                    15,
                    nil,
                    2.5,
                    Vector3.new(8.5, 0.989, 8.5),
                    Character,
                    0,
                    "Slidez"
                )
    ...
    ["Ice Slide"] = function(PathData)
        ...
        while wait() do
            ...
            VfxHandler.FloorFreeze(
                Root.CFrame * CFrame.new(0, 1, -4),
                100,
                10,
                15,
                nil,
                2,
                Vector3.new(9, 0.989, 9),
                Character,
                0,
                "Slide"
            )
            ...
            if Root:FindFirstChild("IceSlide") == nil then
                break
            end
        end
    ...
    ["Ice Age"] = function(PathData)
        ...
        for _ = 1, 7 do
            ...
            GlobalFunctions.FreshShake(65, 25, 1, 0.2, 0)
            ...
            for Index = 1, 13 do
                ...
                local RayCalculation = IceCube.CFrame
                    * CFrame.new(0, 2, -MultiplyNumber)
                    * CFrame.Angles(
                ...
                local RaycastResult = workspace:Raycast(
                    RayCalculation.p + Vector3.new(0, 10, 0),
                    CFrame.new(RayCalculation.p).UpVector * -402,
                    racaystParams
                )
                local Position, Target = RaycastResult.Position, RaycastResult.Instance
                if
                    not Target
                    or Position.Y <= workspace.World.Enviornment.TestPart.Position.Y
                    or Position.Y < workspace.World.Enviornment.TestPart2.Position.Y
                then
                    if
                        Position.Y <= workspace.World.Enviornment.TestPart.Position.Y
                        and workspace.World.Enviornment.TestPart2.Position.Y < Position.Y
                    then
                        Position = Vector3.new(Position.X, workspace.World.Enviornment.TestPart.Position.Y, Position.Z)
                    end
                    ...
                    local ToTweenCFrame = CFrame.new(Position)
                        * CFrame.Angles(
                            math.rad(math.random(-360, 360)),
                            math.rad(math.random(-360, 360)),
                            math.rad(math.random(-360, 360))
                        )
                    local ToSizeIndex = Vector3.new(NumberIndex, NumberIndex, NumberIndex)
                        * (math.random(50, 100) / 100)

                    local Tween = TweenService:Create(
                        IceCube,
                        TweenInfo.new(0.25, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out),
                        { ["CFrame"] = ToTweenCFrame, ["Size"] = ToSizeIndex }
                    )
                    Tween:Play()
                    Tween:Destroy()
                        ...
                        coroutine.wrap(function()
                            ...
                            if Weld == nil then
                                local Tween = TweenService:Create(
                                    ...
                                    { ["CFrame"] = CFrame.new(IceCube.CFrame.p) * CFrame.new(0, -IceCube.Size.X, 0) }
                                )
                                Tween:Play()
                                Tween:Destroy()
                            end
