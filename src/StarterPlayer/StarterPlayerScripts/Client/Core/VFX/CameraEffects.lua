...
local CameraEffects = {
	...
	ChangeCameraType = function(Data)
		...
		if _G.CameraLock == false then return end
		if Data == true then
	...
	CustomBarToValue = function(Data)
		...
		if Player[Data.WhichValue].Value >= 295 then return end

		if BarScreenGui then
			BarScreenGui.Frame.Bar:TweenSize(UDim2.new(Player[Data.WhichValue].Value / 300,0,.588,0),"Out","Quad",.25)
		end
	end,
	...
	MENBEREEEEE = function(Data)
		...
		for _,v in ipairs(PlayerGui.Ultimates:GetDescendants()) do
			if v:IsA("ImageLabel") then
				local Tween = TweenService:Create(v,TweenInfo.new(.15),{ImageTransparency = 1})
				Tween:Play()
				Tween:Destroy()
			...
			elseif v:IsA("TextLabel") then
				local Tween = TweenService:Create(v,TweenInfo.new(.15),{TextTransparency = 1})
				Tween:Play()
				Tween:Destroy()
				coroutine.wrap(function()
					Tween.Completed:Wait()
	...
	Sharingan = function(Data)
		...
		local Tween = TweenService:Create(PlayerGui.Sharingan.Frame.ImageLabel,TweenInfo.new(.75,Enum.EasingStyle.Circular, Enum.EasingDirection.Out),{["Rotation"] = math.random(800), ["ImageTransparency"] = 1})
		Tween:Play()
		Tween:Destroy()

		Tween.Completed:Wait()

		local Tween = TweenService:Create(SharinganColor,TweenInfo.new(.25,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{["TintColor"] = Color3.fromRGB(255,255,255)})
		Tween:Play()
		Tween:Destroy()
	...
	SanjiCutscene = function(Data)
		...
		local Cutscene = TweenService:Create(CurrentCamera,TweenInfo.new(1,Enum.EasingStyle.Quart,Enum.EasingDirection.Out),{CFrame = Root.CFrame * CFrame.Angles(0,math.rad(180),0) * CFrame.new(0,2.35,15)})
		Cutscene:Play()
	...
	--[[local Cutscene = TweenService:Create(CurrentCamera,TweenInfo.new(.35,Enum.EasingStyle.Quart,Enum.EasingDirection.Out),{CFrame = Root.CFrame * CFrame.Angles(0,math.rad(180),0) * CFrame.new(0,-1,10)})
		...
		CurrentCamera.CameraType = Enum.CameraType.Custom
		wait(.1)
		local Tween = TweenService:Create(CurrentCamera,TweenInfo.new(.5,Enum.EasingStyle.Quart,Enum.EasingDirection.Out),{CFrame = InitialCFrame})
		Tween:Play()
		Tween:Destroy()

		local FOV = CurrentCamera.FieldOfView

		local Zoom = TweenService:Create(CurrentCamera,TweenInfo.new(.5,Enum.EasingStyle.Quart,Enum.EasingDirection.Out),{FieldOfView = 180})
		Zoom:Play()
		Zoom:Destroy()

		Zoom.Completed:Wait()

		local Zoom = TweenService:Create(CurrentCamera,TweenInfo.new(.5,Enum.EasingStyle.Quart,Enum.EasingDirection.Out),{FieldOfView = FOV})
		Zoom:Play()
		Zoom:Destroy()
	...
	ChangeUICooldown = function(Data)
		...
		local Tween = TweenService:Create(Gradient,TweenInfo.new(Cooldown,Enum.EasingStyle.Linear,Enum.EasingDirection.Out),{Offset = Vector2.new(-1,0)})
		Tween:Play()
		Tween:Destroy()

		local Tween = TweenService:Create(SkillNameText, TweenInfo.new(.75,Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {TextColor3 = Color3.fromRGB(255, 61, 61)})
		Tween:Play()
		Tween:Destroy()
		...
		local Tween = TweenService:Create(SkillNameText, TweenInfo.new(.75,Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {TextColor3 = Color3.fromRGB(255, 255, 255)})
		Tween:Play()
		Tween:Destroy()
	end;
	...
	ColorCorrection = function(Data)
		if Data.Type == "Create" then
			...
			local Tween = TweenService:Create(ColorCorrection,TweenInfo.new(Data.Length,Enum.EasingStyle.Linear, Enum.EasingDirection.Out),{TintColor = Data.TweenColor})
			Tween:Play()
			Tween:Destroy()
	...
	AddGradient = function(Data)
		...
		if Data.Type == "Add" then
			local Tween = TweenService:Create(Gradient,TweenInfo.new(Data.Length,Enum.EasingStyle.Linear,Enum.EasingDirection.Out),{ImageTransparency = .35})
			Tween:Play()
			Tween:Destroy() 
		...
		elseif Data.Type == "Remove" then
			local Tween = TweenService:Create(Gradient,TweenInfo.new(Data.Length,Enum.EasingStyle.Linear,Enum.EasingDirection.Out),{ImageTransparency = 1})
			Tween:Play()
			Tween:Destroy()
	...
	TweenObject = function(Data)
		local Tween = TweenService:Create(Data.Object or workspace.CurrentCamera,TweenInfo.new(Data.LifeTime,Data.EasingStyle,Data.EasingDirection,0,Data.Return,0),Data.Properties)
		Tween:Play()
		Tween:Destroy()
	end;
	...
	BreakCooldown = function(Data)
		...
		for _,v in ipairs(Slots:GetDescendants()) do
			if v:FindFirstChild("CooldownGradient") then
				repeat RunService.RenderStepped:Wait() v.CooldownGradient.Offset = Vector2.new(-1, 0) until v.CooldownGradient.Offset == Vector2.new(-1, 0) 
