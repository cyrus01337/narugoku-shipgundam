...
local PlayerClient = {
    ...
    Dash = function(Data)
        ...
        VfxHandler.AfterImage({ Character = Character, Duration = 0.3, StartTransparency = 0.25 })
    ...
    LightningStun = function(BackData)
        ...
        local Parents = {
            ...
            Victim:FindFirstChild("Head"),
            Victim:FindFirstChild("Left Arm"),
            Victim:FindFirstChild("Right Leg"),
        }
        ...
        for _, BodyParts in ipairs(Victim:GetChildren()) do
            ...
            Debris:AddItem(LightningParticle, 0.75)
        end

        if Animator then
            local Animation = Animator:LoadAnimation(
                ReplicatedStorage.Assets.Animations.Shared.Combat.HitReaction["HitReaction" .. (math.random(1, 2))]
        ...
        Attach2.Position = Vector3.new(0.5, 0.5, 0.5)

        local Bolts = LightningModule.new(Attach, Attach2, 50)
        Bolts.PulseLength = 0.8
