...
local SanjiVFX = {
    ...
    ["AscendTrail"] = function(PathData)
        ...
        if GlobalFunctions.CheckDistance(Player, 15) then
            GlobalFunctions.FreshShake(180, 100, 1, 0.15)
        end
    ...
    ["Anti_Manner_Kick_Course"] = function(PathData)
        ...
        if GlobalFunctions.CheckDistance(Player, 25) then
            GlobalFunctions.FreshShake(185, 35, 1, 0.15)
        end
    ...
    ["Coiler"] = function(PathData)
        ...
        if GlobalFunctions.CheckDistance(Player, 30) then
            GlobalFunctions.FreshShake(120, 25, 1, 0.15)
        end
    ...
    ["Transformation"] = function(PathData)
            ...
            while true do
                if os.clock() - StartTime >= 0.1 then
                    ...
                    local Tween = TweenService:Create(
                        Slash.Decal,
                        TweenInfo.new(0.25, Enum.EasingStyle.Sine, Enum.EasingDirection.Out),
                        { ["Transparency"] = 1, ["Color3"] = Color3.fromRGB(0, 0, 0) }
                    )
                    Tween:Play()
                    Tween:Destroy()
                    ...
                    local Tween = TweenService:Create(
                        Slash.Mesh,
                        TweenInfo.new(0.25, Enum.EasingStyle.Sine, Enum.EasingDirection.Out),
                        { ["Scale"] = Vector3.new(-30, -1, -30) }
                    )
                    Tween:Play()
                    Tween:Destroy()

                    local CFrameDebounce = nil
                    CFrameDebounce = returnCFrame(CFrame.Angles(0, -0.25, 0), 0.02 * 60)
        ...
        FlameLeg.CFrame = Character["Right Leg"].CFrame * CFrame.Angles(math.rad(-90), 0, 0) * CFrame.new(0, 0, -0.35)
