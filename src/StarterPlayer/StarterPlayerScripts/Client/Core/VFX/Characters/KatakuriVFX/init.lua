...
local function MochiGround(Character, Amount, EndPosition, Force)
    for _ = 1, Amount do
        ...
        ball.CFrame = CFrame.new(EndPosition)
            * CFrame.new(0, 2, 0)
            * CFrame.Angles(math.rad(90), 0, 0)
            * CFrame.Angles(math.rad(math.random(-45, 45)), math.rad(math.random(-45, 45)), 0)
        ...
        conn = ball.Touched:Connect(function(hit)
            if
                ...
                and not hit:IsDescendantOf(Visuals)
                and not hit:IsDescendantOf(World.Live)
            ...
            then
                ...
                local tween = TweenService:Create(ball, TweenInfo.new(0.5), { Size = Vector3.new(size, 0.25, size) })
                tween:Play()
                tween:Destroy()
                coroutine.wrap(function()
                    wait(0.75)
                    local tween =
                        TweenService:Create(ball, TweenInfo.new(2), { Transparency = 1, Color = DoughEndColour })
                    tween:Play()
                    tween:Destroy()
...
local IchigoVFX = {
    ...
    ["MoguraFlow"] = function(PathData)
        ...
        while true do
            ...
            if os.clock() - StartTime >= 0.75 then
                break
            end
        end
        Dust.Attachment.dust1.Enabled = false
    ...
    ["MochiRoll"] = function(PathData)
        ...
        Donut.CFrame = Character.HumanoidRootPart.CFrame
            * CFrame.new(0, 5, 0)
        ...
        coroutine.wrap(function()
            ...
            while BodyVelocity.Parent do
                ...
                if GlobalFunctions.CheckDistance(Player, math.random(35, 45)) then
                    GlobalFunctions.FreshShake(20, 20, 0.1, 0.1, 0)
                end
                wait(0.35)
            end
        ...
        while Weld.Parent do
            ...
            if ray then
                ...
                if partHit then
                    ...
                    BodyVelocity.Velocity = Vector3.new(math.random(-40, 40), math.random(30, 40), math.random(-40, 40))
                        * 0.65
    ...
    ["FlowingMochi"] = function(PathData)
        ...
        local function DragonImpact(Dragon)
            ...
            local tween = TweenService:Create(
                shock,
                TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                { Size = Vector3.new(30, 50, 30), CFrame = shock.CFrame
                    * CFrame.new(0, 20, 0)
            ...
            coroutine.wrap(function()
                ...
                local tween = TweenService:Create(
                    ...
                    { Size = Vector3.new(0, 80, 0), Color = DoughEndColour }
                )
                tween:Play()
                tween:Destroy()
                Debris:AddItem(shock, 0.2)
            end)()
            ...
            Shockwave.Shockwave.Size =
                NumberSequence.new({ NumberSequenceKeypoint.new(0, 0), NumberSequenceKeypoint.new(1, 80) })
        ...
        for Index = 1, Magnitude, Speed do
            ...
            local nextPoint = (
                (Index + 1) < Magnitude
                    and BezierModule:quadBezier((Index + 1) / Magnitude, StartPosition, PointA, EndPosition)
                or BezierModule:quadBezier(1, StartPosition, PointA, EndPosition)
            )

            -- trail test --
            local ChangeRate = 2
            local DirectionOffset = CFrame.new(
                math.random(-ChangeRate, ChangeRate),
                math.random(-ChangeRate, ChangeRate),
                math.random(-ChangeRate, ChangeRate)
            )
            ...
            local tween = TweenService:Create(
                ...
                { Size = Vector3.new(0, trail.Size.Y + 8, 0), Color = Color3.fromRGB(190, 182, 158) }
            )
            tween:Play()
            tween:Destroy()
