...
local TanjiroVFX = {
	...
	["Striking Tide"] = function(PathData)
		...
		coroutine.resume(coroutine.create(function()
			...
			local SlashEffect3D = ReplicatedStorage.Assets.Effects.Meshes.ThreeDSlashEffect:Clone(); SlashEffect3D.Anchored = true; SlashEffect3D.CanCollide = false; SlashEffect3D.Massless = true; SlashEffect3D.CFrame =  Root.CFrame * CFrame.Angles(math.pi-.2,-math.pi/2.3,(math.pi/2)-.3); SlashEffect3D.Parent = workspace.World.Visuals;

			local Tween = TweenService:Create(SlashEffect3D,B5,{Transparency = 1,CFrame = SlashEffect3D.CFrame*CFrame.Angles(3.36,0,0),Size = SlashEffect3D.Size+Vector3.new(0,SizeAddon,SizeAddon)})			
			Tween:Play()
			Tween:Destroy()
		...
		VfxHandler.Orbies({Parent = Root, Speed = .5, Size = Vector3.new(1, 1, 12), Cframe = CFrame.new(0,0,15), Amount = 8, Sphere = true})
		...
		GlobalFunctions.TweenFunction({	["Instance"] = Ring,["EasingStyle"] = Enum.EasingStyle.Quad,["EasingDirection"] = Enum.EasingDirection.Out,["Duration"] = .5,},{["Transparency"] = 1; ["Size"] = Vector3.new(25,.05,25); ["CFrame"] = Ring.CFrame * CFrame.new(0,12,0) })	

		local SlashEffect = ReplicatedStorage.Assets.Effects.Meshes["3dSlashEffect"]:Clone(); SlashEffect.Anchored = true; SlashEffect.CanCollide = false; SlashEffect.Massless = true; SlashEffect.CFrame = Character.HumanoidRootPart.CFrame * CFrame.new(0,-1,0) * CFrame.Angles(math.rad(52.12),math.rad(52.68),math.rad(-16.19)); SlashEffect.Parent = workspace.World.Visuals;

		GlobalFunctions.TweenFunction({ ["Instance"] = SlashEffect, ["EasingStyle"] = Enum.EasingStyle.Quad, ["EasingDirection"] = Enum.EasingDirection.Out, ["Duration"] = .4, },{ ["Size"] = Vector3.new(4.15, 0.538, 3.075); ['CFrame'] = SlashEffect.CFrame * CFrame.fromEulerAnglesYXZ(0,-10,0); })		
		GlobalFunctions.TweenFunction({ ["Instance"] = SlashEffect.Decal, ["EasingStyle"] = Enum.EasingStyle.Quad, ["EasingDirection"] = Enum.EasingDirection.Out, ["Duration"] = .4,},{ ['Transparency'] = 1; })

		GlobalFunctions.TweenFunction({["Instance"] = SlashEffect.Mesh,["EasingStyle"] = Enum.EasingStyle.Quad,["EasingDirection"] = Enum.EasingDirection.Out,["Duration"] = .75,},{["Scale"] = Vector3.new(-14.988, 2.613, 14.988);})
	...
	["WaterWheelHit"] = function(BackData)
		...
		VfxHandler.Orbies({Parent = VRoot, Speed = .5, Size = Vector3.new(1, 1, 8), Cframe = CFrame.new(0,0,11), Amount = 8, Sphere = true})
	...
	["Whirlpool"] = function(Data)
		...
		local WhirlpoolTweenInfo = TweenInfo.new(.35,Enum.EasingStyle.Linear,Enum.EasingDirection.Out,0,false,0)
		...
		if Character and Cframe then
			...
			coroutine.resume(coroutine.create(function()
				for Index = 1,6 do
					...
					TweenService:Create(Ring,WhirlpoolTweenInfo,{CFrame = Cframe,Size = Vector3.new(5,3,5)}):Play()

					wait(.1)
				end
			end))

			TweenService:Create(Swirl,TweenInfo.new(.25,Enum.EasingStyle.Linear,Enum.EasingDirection.Out,0,false,0),{
				Size = Vector3.new(30,48,30),
				CFrame = Swirl.CFrame * CFrame.new(0,26,0)
			}):Play()

			TweenService:Create(Swirl,TweenInfo.new(.475,Enum.EasingStyle.Quad, Enum.EasingDirection.Out,1,true,.35),{
			...
			coroutine.resume(coroutine.create(function()
				for _ = 1,8 do
					...
					GlobalFunctions.TweenFunction({["Instance"] = Slice,["EasingStyle"] = Enum.EasingStyle.Quad,["EasingDirection"] = Enum.EasingDirection.Out,["Duration"] = .6,},{["Size"] = Vector3.new(6, 0.2, 10);['Transparency'] = 1;['CFrame'] = Slice.CFrame * CFrame.fromEulerAnglesYXZ(0,10,0);})
					wait(Data.Duration or .35)	
				end
			end))

			coroutine.resume(coroutine.create(function()
				wait(.5)
				TweenService:Create(Swirl,TweenInfo.new(.4,Enum.EasingStyle.Linear,Enum.EasingDirection.Out,0,false,0),{
			...
			while Swirl do
				...
				if workspace.World.Visuals:FindFirstChild(Character.Name.." tanjiro whirlpool swirl") == nil then break end
			end
		end	
	end,
