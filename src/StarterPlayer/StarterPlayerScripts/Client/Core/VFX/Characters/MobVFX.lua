...
local MobVFX = {
	...
	["Physco Rocks"] = function(Data)
		...
		for Index = 1, #Rocks do
			...
			RandomRockModel.Orientation = Vector3.new(math.random(-360,360),math.random(-360,360),math.random(-360,360))

			delay(RockData.Delay,function()
				local Animation = TweenService:Create(RandomRockModel, TweenInfo.new(AnimationDuration/3 - (os.clock() - Timestamp)), {Size = Vector3.new(1,1,1) * RockData.Size})
				Animation:Play()
				Animation:Destroy()
			end)
		...
		for Index = 1, #NewRocks do
			...
			RaycastManager:CastProjectileHitbox({
				...
				Function = function(RaycastResult) 
					Explosions.PhyscoRocks({Index = Index, Rock = Rock, Character = Character, RaycastResult = RaycastResult, Distance = Data.Distance, Size = Size})
			...
			local Animate = TweenService:Create(Rock, TweenInfo.new(RockLifetime, Enum.EasingStyle.Linear), {CFrame = CFrame.new(Rock.Position + Direction * RockVelocity * RockLifetime)})
			Animate:Play()
			Animate:Destroy()
			wait(.05)
		end
	...
	["PhyscoChoke"] = function(Data)
		...
		VfxHandler.Orbies({Parent = VRoot, Speed = .35, Cframe = CFrame.new(0,0,3), Amount = 3, Circle = true})
	...
	["PhyscoMeteor"] = function(PathData)
		...
		coroutine.resume(coroutine.create(function()
			for Index = 1,2 do			
				...
				local Tween = TweenService:Create(Ring,TweenInfo.new(.4,Enum.EasingStyle.Quad,Enum.EasingDirection.Out,0,false,0),{CFrame = Ring.CFrame * CFrame.new(0,15,0) ,Size = Vector3.new(0,0,0)})
				Tween:Play()
				Tween:Destroy()
		...
		local Animate = TweenService:Create(Projectile, TweenInfo.new(LifeTime, Enum.EasingStyle.Linear), {CFrame = Projectile.CFrame * CFrame.new(0, 0, -Velocity * LifeTime)})
		Animate:Play()
		Animate:Destroy()
	...
	["PhyscoSlamUp"] = function(PathData)
		...
		local Result = RayService:Cast(Victim.HumanoidRootPart.Position + Vector3.new(0,50,0), Victim.HumanoidRootPart.Position - Vector3.new(0,1000,0), {workspace.World.Live, workspace.World.Visuals}, Enum.RaycastFilterType.Exclude)
		...
		coroutine.resume(coroutine.create(function()
			for _ = 1,5 do		
				VfxHandler.AfterImage({Character = Victim, Duration = 1, StartTransparency = .2, Color = Color3.fromRGB(84, 42, 135) })
				wait()
			end
		end))
	...
	["PhyscoSlamDown"] = function(PathData)
		...
		for _ = 1,2 do		
			VfxHandler.AfterImage({Character = Victim, Duration = 1, StartTransparency = .2, Color = Color3.fromRGB(100, 49, 150) })
			wait()
		end
