...
local GilgameshVFX = {
	...
	["OpenPortal"] = function(PathData)
		...
		for _, Particle in ipairs(Portal:GetDescendants()) do
			if Particle:IsA("ParticleEmitter") then
				Particle:Emit(1.25)
				delay(1.25,function() Particle.Enabled = false end)
			end
		end
		...
		for _ = 1 ,2 do
			...
			Attach.Position = Vector3.new(-.5,5,.85);
			...
			local Bolts = LightningModule.new(Attach,Attach2,50); Bolts.PulseLength = .35; Bolts.Color = Color3.fromRGB(255, 233, 121);
	...
	["Zashu"] = function(PathData)
		...
		RaycastService:CastProjectileHitbox({
			...
			Function = function(RaycastResult)
				Explosions.Zashu({Character = Character, RaycastResult = RaycastResult, Spear = SwordClone, Distance = PathData.Distance})
	...
	["GatesOfBabylon"] = function(PathData)
		...
		for Index = 1, 7 do
			...
			SwordClone.CFrame = CFrame.new(CurrentStartPoint.Position, MouseHit.Position) * CFrame.Angles(math.rad(90),math.rad(90),math.rad(180))
			...
			local InitialTween = TweenService:Create(SwordClone, TweenInfo.new(.5, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out, 0, false, 0), {["CFrame"] = SwordClone.CFrame * CFrame.new(0, 5,0) * CFrame.fromEulerAnglesXYZ(0, 4, 0)})
			InitialTween:Play()
			InitialTween:Destroy()
			...
			coroutine.resume(coroutine.create(function()
				...
				RaycastService:CastProjectileHitbox({
					...
					Visualize = false,
					Function = function(RaycastResult)
						Explosions.GatesOfBabylon({Index = Index, Character = Character, RaycastResult = RaycastResult, Spear = SwordClone, Distance = PathData.Distance})
	...
	["Enkidu"] = function(PathData)
		...
		for Index = 1, 4 do
			...
			local Animate = TweenService:Create(BeamClone.BeamEnd, TweenInfo.new(Lifetime / 3, Enum.EasingStyle.Linear), {["WorldPosition"] = (SwordClone.CFrame * CFrame.new(0,Velocity * Lifetime,0)).Position})
			Animate:Play()
			Animate:Destroy()
	...
	["Punishment"] = function(PathData)
		...
		for Index = 1, #StartPositionData do
			...
			local SwordClone = Assets.Models.Swords.GilgameshSpear:Clone()
			SwordClone.CFrame = CurrentStartPoint 
			SwordClone.CFrame = CFrame.new(CurrentStartPoint.Position, Target.PrimaryPart.Position) * CFrame.Angles(math.rad(90),math.rad(90),math.rad(180))
			...
			local InitialTween = TweenService:Create(SwordClone, TweenInfo.new(.75, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out, 0, false, 0), {["CFrame"] = SwordClone.CFrame * CFrame.new(0, 5,0) * CFrame.fromEulerAnglesXYZ(0, 4, 0)})
			InitialTween:Play()
			InitialTween:Destroy()
			...
			coroutine.resume(coroutine.create(function()
				...
				RaycastService:CastProjectileHitbox({
					...
					Visualize = false,
					Function = function(RaycastResult)
						Explosions.GatesOfBabylon({Index = Index, Character = Character, RaycastResult = RaycastResult, Spear = SwordClone, Distance = PathData.Distance, Type = "Punishment"})
