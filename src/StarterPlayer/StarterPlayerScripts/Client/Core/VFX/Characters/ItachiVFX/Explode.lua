...
return function(MenbereCFrame, MenbereCFrame2, Menbere, RaycastResult)
	...
	local Tween = TweenService:Create(DustEffect.PointLight, TweenInfo.new(.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Range"] = 40,  Brightness = 0})
	Tween:Play()
	Tween:Destroy()
	...
	for _ = 1, 10 do
		...
		local Tween = TweenService:Create(Sphere, TweenInfo.new(.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			["CFrame"] = Sphere.CFrame * CFrame.new(math.random(-3000, 3000) / 100, math.random(-3000, 3000) / 100, math.random(-3000, 3000) / 100)
		})
		Tween:Play()
		Tween:Destroy()
		
		local Tween = TweenService:Create(Sphere.Mesh, TweenInfo.new(.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Scale"] = Vector3.new(0,0,0)})
		Tween:Play()
		Tween:Destroy()
		...
		local Tween = TweenService:Create(Sphere2, TweenInfo.new(.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			["CFrame"] = Sphere2.CFrame * CFrame.new(math.random(-3000, 3000) / 100, math.random(-3000, 3000) / 100, math.random(-3000, 3000) / 100)
		})
		Tween:Play()
		Tween:Destroy()
		
		local Tween = TweenService:Create(Sphere2.Mesh, TweenInfo.new(.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Scale"] = Vector3.new(0,0,0)})
		Tween:Play()
		Tween:Destroy()
		
		Debris:AddItem(Sphere2, .35)
	end
end