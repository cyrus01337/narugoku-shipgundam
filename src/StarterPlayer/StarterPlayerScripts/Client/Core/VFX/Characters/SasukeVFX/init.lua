...
camShake = require(ReplicatedStorage.Modules.Effects.CameraShaker).new(
...
local function urmom(Particle)
    ...
    Clone.Enabled = false
    Clone.Lifetime = NumberRange.new(0.25, 0.5)
...
function Current(Raycast, Part2, Length, Spread, Size)
    local Size = math.random((Size * 0.5), Size * 10) / 10
    ...
    Part.Attachment2.Position = Vector3.new(-math.random((Size * 0.25) * 100, Size * 100) / 100, 0, 0)
    Part.Attachment1.Position = Vector3.new(math.random((Size * 0.25) * 100, Size * 100) / 100, 0, 0)
    ...
    for _ = 0, MagnitudeIndex, MagnitudeIndex * (Length / 100) do
        ...
        local X, Y, Z =
            math.random(-Spread, Spread) / 10, math.random(-Spread, Spread) / 10, math.random(-Spread, Spread) / 10
...
function Activate(Raycast, Color, Voltage, SP)
    for _ = 1, Voltage do
        ...
        local CF2 = CFrame.new(EndPosition, Raycast.Origin)
            * CFrame.new(
                math.random(-SP.Extra.End, SP.Extra.End),
                math.random(-SP.Extra.End, SP.Extra.End),
                math.random(SP.Extra.End, SP.Extra.End * 1.95)
            )
        EndPosition = CF2.Position
        local CF = CFrame.new(Raycast.Origin, EndPosition)
            * CFrame.new(
                math.random(-SP.Extra.Start, SP.Extra.Start),
                math.random(-SP.Extra.Start, SP.Extra.Start),
                math.random(SP.Extra.Start * 0.8, SP.Extra.Start)
            )
...
function DirtEffect(Pos, Character, pos2)
    ...
    if Target then
        ...
        for _ = 1, 1 do
            ...
            Table[#Table + 1] = Rock
            Rock.Position = Position + Vector3.new(math.random(-10, 10) / 10, -0.5, math.random(-10, 10) / 10)
            Rock.Color = Target.Color
            Rock.Material = Target.Material
            Rock.BodyAngularVelocity.AngularVelocity =
                Vector3.new(math.random(-4, 4), math.random(-4, 4), math.random(-4, 4))

            local Tween = TweenService:Create(
                Rock,
                TweenInfo.new(2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                { ["Size"] = Vector3.new(), ["Transparency"] = 1 }
            )
            Tween:Play()
            Tween:Destroy()
            ...
            Debris:AddItem(Rock.BodyVelocity, 0.1)
        end
        delay(3, function()
...
local SasukeVFX = {
    ["ShurikenHit"] = function(PathData)
        ...
        for Index = 1, math.random(3, 5) do
            ...
            BloodBullet.Velocity = Vector3.new(0, 0, 0)
                + Vector3.new(math.random(-15, 15), math.random(15, 50), math.random(-15, 15))
            ...
            local Connection
            Connection = BloodBullet.Touched:Connect(function(Hit)
                if
                    not Hit:IsDescendantOf(workspace.World.Live)
                    and not Hit:IsDescendantOf(workspace.World.Visuals)
                    and not Hit:IsDescendantOf(workspace.CurrentCamera)
                then
                    BloodBullet.Anchored = true
                    BloodBullet.CanCollide = false
                    local CFrameCalc = BloodBullet.CFrame * CFrame.new(0, 10, 0)
                    local Target, Position, Surface = workspace:FindPartOnRayWithIgnoreList(
                        Ray.new(CFrameCalc.p, CFrameCalc.upVector * -25),
                        { workspace.World.Live, workspace.World.Visuals }
                    )
                    if Target then
                        local BloodStain = script.blood:Clone()
                        BloodStain.CFrame = CFrame.new(Position, Position + Surface)
                            * CFrame.Angles(math.rad(90), math.rad(math.random(-360, 360)), 0)
                        ...
                        local Tween = TweenService:Create(
                            ...
                            { Size = Vector3.new(SizeCalc, 0, SizeCalc) }
                        )
                        Tween:Play()
                        Tween:Destroy()
                    ...
                    Debris:AddItem(BloodBullet, 0.75)
    ...
    ["Fireball Jutsu"] = function(PathData)
        ...
        if
            ...
            and (RaycastResult.Position - PositionCalculation).Magnitude < 30
        then
            ...
            LeftDust.CFrame = End * CFrame.Angles(0, 0.3, 0) * CFrame.new(3, 0, 0)
            ...
            RightDust.CFrame = End * CFrame.Angles(0, -0.3, 0) * CFrame.new(-3, 0, 0)
        ...
        for Index = 1, 100 do
            ...
            local Tween = TweenService:Create(
                Part.Mesh,
                TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                { ["Scale"] = Vector3.new(0, MagnitudeIndex, 0) }
            )
            Tween:Play()
            Tween:Destroy()
    ...
    ["Chidori"] = function(PathData)
        ...
        local Tween = TweenService:Create(
            ...
            { ["Transparency"] = 1, ["Size"] = Vector3.new(20, 20, 20) }
        )
        Tween:Play()
        Tween:Destroy()
        ...
        coroutine.wrap(function()
            for _ = 1, 48 do
                ...
                local Tween = TweenService:Create(
                    B,
                    TweenInfo.new(0.6, Enum.EasingStyle.Quint, Enum.EasingDirection.Out, 0, false, 0),
                    { ["Range"] = RandomIndex, ["Brightness"] = 0 }
                )
                Tween:Play()
                Tween:Destroy()
        ...
        Activate(
            ...
            "Blue",
            20,
            { Extra = { Start = 20, End = 20 }, Bolt = { Spread = 20, Size = 3, Length = 10 } }
        )
    ...
    ["ChidoriHit"] = function(PathData)
        ...
        VFXHandler.Orbies({
            Parent = Victim.HumanoidRootPart,
            Speed = 0.35,
        ...
        for _ = 1, 6 do
            ...
            Base2.CFrame = ChidoriExplosion.CFrame
                * CFrame.new(0, 0, 0)
                * CFrame.Angles(math.random(-999, 999), math.random(-999, 999), math.random(-999, 999))
            ...
            local Tween = TweenService:Create(
                ...
                { Transparency = 1, Size = Vector3.new(RandomIndex, RandomIndex, RandomIndex) }
            )
            Tween:Play()
            Tween:Destroy()
        ...
        local Tween = TweenService:Create(
            ChidoriExplosion.ChidoriGlow,
            TweenInfo.new(1, Enum.EasingStyle.Quint, Enum.EasingDirection.Out, 0, false, 0),
            { Brightness = 0, Range = 40 }
        )
        Tween:Play()
        Tween:Destroy()
