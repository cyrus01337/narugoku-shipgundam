...
local WhitebeardVFX = {
	["QuakePunch"] = function(PathData)
		...
		local tween = TweenService:Create(GuraHand, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Transparency"] = 1})
		tween:Play()
		tween:Destroy()
		...
		local tween = TweenService:Create(GuraCrack, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Size"] = Vector3.new(20,.1,20)})
		tween:Play()
		tween:Destroy()
		coroutine.wrap(function()
			wait(1)
			local tween = TweenService:Create(GuraCrack, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Transparency"] = 1})
			tween:Play()
			tween:Destroy()
		end)()
		...
		coroutine.wrap(function()
			while GuraBall.Parent do
				...
				local c1,c2 = GuraBall.CFrame*CFrame.new(0,0,-1)*CFrame.Angles(math.pi/2,0,0) ,GuraBall.CFrame*CFrame.new(0,0,10)*CFrame.Angles(math.pi/2,0,0) 
				cs.CFrame = c1
				cs.Material = Enum.Material.Neon
				cs.Parent = workspace.World.Visuals

				local Tween = TweenService:Create(cs,TweenInfo.new(.15,Enum.EasingStyle.Quad,Enum.EasingDirection.Out,0,false,0),{Transparency = 1, Size = Vector3.new(25,0,25),CFrame = c2})
				Tween:Play()
				Tween:Destroy()
		...
		coroutine.wrap(function()
			while GuraBall.Parent do
				...
				local Tween = TweenService:Create(classicshockwave,TweenInfo.new(.15,Enum.EasingStyle.Quad,Enum.EasingDirection.Out,0,false,0),{Transparency = 1, Size = Vector3.new(50, 2, 50)})
				Tween:Play()
				Tween:Destroy()

				Debris:AddItem(classicshockwave,.15)
				wait(.1)
			end
		end)()
		...
		for j = 1,2 do
			...
			local tween = TweenService:Create(SideWind, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["CFrame"] = SideWind.CFrame * CFrame.new(-10,0,0), ["Size"] = GoalSize, ["Transparency"] = 1})
			tween:Play()
			tween:Destroy()
		...
		coroutine.wrap(function()
			for j = 1,10 do
				for i = 1,2 do
					...
					local tween = TweenService:Create(beam, TweenInfo.new(.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Size"] = beam.Size + Vector3.new(0,0, math.random(1,2)), ["CFrame"] = beam.CFrame * CFrame.new(0,0,math.random(20,25))})
					local tween2 = TweenService:Create(beam, TweenInfo.new(.15, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {["Size"] = Vector3.new(0,0,math.random(3,6))})		
					tween:Play()
					tween:Destroy()
					tween2:Play()
					tween2:Destroy()
		...
		coroutine.wrap(function()
			...
			local tween = TweenService:Create(GuraBall, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Size"] = GuraBall.Size + Size})
			tween:Play()
			tween:Destroy()

			local tween = TweenService:Create(GuraBall.Core, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Size"] = GuraBall.Core.Size + Size})
			tween:Play()
			tween:Destroy()
			
			wait(0.15)
			local tween = TweenService:Create(GuraBall, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Size"] = GuraBall.Size - Size})
			tween:Play()
			tween:Destroy()

			local tween = TweenService:Create(GuraBall.Core, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Size"] = GuraBall.Core.Size - Size})
			tween:Play()
			tween:Destroy()
		end)()
		--[[ Tween Slash ]]--
		...
		local tween = TweenService:Create(GuraBall, TweenInfo.new(Lifetime, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {["CFrame"] = GuraBall.CFrame * CFrame.new(0,0,-Distance)})
		tween:Play()
		tween:Destroy()
		
		local tween = TweenService:Create(GuraBall.Core, TweenInfo.new(Lifetime, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {["CFrame"] = GuraBall.Core.CFrame * CFrame.new(0,0,-Distance)})
		tween:Play()
		tween:Destroy()
		...
		wait(Lifetime)
		local tween = TweenService:Create(GuraBall, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Size"] = Vector3.new(0,0,0)})
		tween:Play()
		tween:Destroy()

		local tween = TweenService:Create(GuraBall.Core, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Size"] = Vector3.new(0,0,0)})
		tween:Play()
		tween:Destroy()
	end;
	...
	["HelmetSplitter"] = function(PathData)
		...
		local function GroundTouched(RootPos, PartHit)
			...
			Smoke.Smoke.Size = NumberSequence.new{NumberSequenceKeypoint.new(0, 1), NumberSequenceKeypoint.new(1, 0)}
			...
			for i = 1,10 do
				...
				if ray then
					...
					if partHit then
						...
						BodyVelocity.Velocity = Vector3.new(math.random(-80,80),math.random(50,60),math.random(-80,80)) * (i*1)
						BodyVelocity.P = 100000
						Block.Velocity = Vector3.new(math.random(-80,80),math.random(50,60),math.random(-80,80)) * (i*1)
			...
			local Smoke = Particles.Smoke:Clone()
			Smoke.Smoke.Size = NumberSequence.new{NumberSequenceKeypoint.new(0, 0), NumberSequenceKeypoint.new(1, 2)}
			Smoke.Smoke.Drag = 5
			Smoke.Smoke.Lifetime = NumberRange.new(3)
		...
		for q = 1,2 do
			...
			local tween = TweenService:Create(Ball, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Transparency"] = 1, ["Size"] = Ball.Size * 5})
			tween:Play()
			tween:Destroy()
			...
			--[[ Gura Crack ]]--
			local GuraCrack = workspace.GuraCrack:Clone()
			GuraCrack.CFrame = CFrame.new(RootPos) * CFrame.new(0,-2.5,0) * CFrame.fromEulerAnglesXYZ(math.rad(180),0,0)
			GuraCrack.Parent = Visuals

			local tween = TweenService:Create(GuraCrack, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Size"] = Vector3.new(20*q,.1,20*q)})
			tween:Play()
			tween:Destroy()
			coroutine.wrap(function()
				wait(1)
				local tween = TweenService:Create(GuraCrack, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Transparency"] = 1})
				tween:Play()
				tween:Destroy()
			end)()
