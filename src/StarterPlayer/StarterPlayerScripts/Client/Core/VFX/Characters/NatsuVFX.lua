...
local NatsuVFX = {
	...
	["Fire Dragon's Iron Fist"] = function(Data)
		...
		coroutine.wrap(function()
			...
			for j = 1,10 do
				for i = 1,5 do
					...
					local originalPos = CFrame.new(RootPosition.Position + Vector3.new(math.random(-1,1)*10,math.random(-1,1)*10,math.random(-1,1)*10), RootPosition.Position)
		...
		coroutine.wrap(function()
			...
			for j = 1,2 do
				for i = 1, 2 do
					...
					local tween = TweenService:Create(Ring, TweenInfo.new(WaitTime, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Size"] = Vector3.new(0,0,0)})
					tween:Play()
					tween:Destroy()
		...
		Attachment.Fire.Size = NumberSequence.new{NumberSequenceKeypoint.new(0, 10), NumberSequenceKeypoint.new(1, 20)}
		...
		coroutine.wrap(function()
			for j = 1,2 do
				for i = 1, 2 do
					...
					local tween = TweenService:Create(Ring, TweenInfo.new(.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Transparency"] = 1, ["Size"] = Vector3.new(50,0,50)})
					tween:Play()
					tween:Destroy()
	...
	["CrimsonLotusStart"] = function(Data)
		...
		coroutine.wrap(function()
			for i = 1,2 do
				...
				Attachment.Fire.Size = NumberSequence.new{NumberSequenceKeypoint.new(0, 7), NumberSequenceKeypoint.new(1, 0)}
			...
			coroutine.wrap(function()
				...
				for i = 1,3 do
					...
					Attachment.Fire.Size = NumberSequence.new{NumberSequenceKeypoint.new(0, 8), NumberSequenceKeypoint.new(1, 12)}
					Attachment.Fire.Acceleration = Vector3.new(0,0,0)
					Attachment.Fire.Rate = 200
			...
			coroutine.wrap(function()
				...
				for j = 1,3 do
					for i = 1,5 do
						...
						local originalPos = CFrame.new(RootPosition.Position + Vector3.new(math.random(-3,3)*10,math.random(-3,3)*10,math.random(-3,3)*10), RootPosition.Position)
						local beam = EffectMeshes.Block:Clone()
						beam.Shape = "Block"
						local mesh = Instance.new("SpecialMesh")
						mesh.MeshType = "Sphere"
	...
	["CrimsonLotusLand"] = function(Data)
		...
		Fire.Attachment.Fire.Size = NumberSequence.new{NumberSequenceKeypoint.new(0, 20), NumberSequenceKeypoint.new(1, 0)}
	...
	["FireDragonRoar"] = function(Data)
		...
		for Index = 0,7 do
			local RingInnit = EffectMeshes.RingInnit:Clone()
			RingInnit.CFrame = Character.HumanoidRootPart.CFrame * CFrame.new(0,0,-(Index * 10)) * CFrame.fromEulerAnglesXYZ(math.rad(90),0,0)
	...
	["PurgatoryDragonFire"] = function(Data)
		...
		Attachment.Fire.Drag = 5
		Attachment.Fire.Lifetime = NumberRange.new(.25,.35)
		Attachment.Fire.Size = NumberSequence.new{NumberSequenceKeypoint.new(0, 1), NumberSequenceKeypoint.new(1, 0)}
	...
	["NatsuScreen"] = function(Data)
		...
		local tween2 = TweenService:Create(ColorCorrection, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Brightness"] = 0, ["Contrast"] = 0, ["Saturation"] = 0})
		tween2:Play()
		tween2:Destroy()
