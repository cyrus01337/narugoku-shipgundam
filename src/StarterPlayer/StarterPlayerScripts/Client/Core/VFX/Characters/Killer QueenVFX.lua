...
local Killer_QueenVFX = {
	...
	["Add Bomb"] = function(Data)
		...
		VfxHandler.Orbies({Parent = VRoot, Speed = .5, Size = Vector3.new(.2, .3, 4.79), Cframe = CFrame.new(0,0,5), Amount = 5, Circle = true, Sphere = true})
	...
	["Switch Bomb"] = function(Data)
		...
		-- SoundManager:AddSound("Explosion", {Parent = Root, Volume = 1}, "Client")
		Explosions.CoinExplosion({Character = Character, ContactPointCFrame = Data.Victim:FindFirstChild("HumanoidRootPart").CFrame, Coin = Data.Victim:FindFirstChild("HumanoidRootPart"), Distance = Data.Distance})
	end,

	["Coin Flip"] = function(Data)
		local Coin = Data.Coin
		for _ = 1,25 do
			local Tweeninfo = TweenInfo.new(.45,Enum.EasingStyle.Linear,Enum.EasingDirection.Out,1,true,0)
	...
	["Coin Toss"] = function(Data)
		...
		local Tween = TweenService:Create(Coin,TweenInfo.new(.475,Enum.EasingStyle.Quad, Enum.EasingDirection.Out),{Color = Color3.fromRGB(255, 62, 62)})
		Tween:Play()
		Tween:Destroy()
		...
		wait(.35)
		-- SoundManager:AddSound("Explosion", {Parent = Root, Volume = 1}, "Client")
		Explosions.CoinExplosion({Character = Character, ContactPointCFrame = Coin.CFrame, Coin = Coin, Distance = Data.Distance})
	end,
