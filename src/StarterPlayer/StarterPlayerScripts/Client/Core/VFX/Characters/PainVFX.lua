...
local PainVFX = {
	["AlmightyPush"] = function(PathData)
		...
		for i = 1, 10 do
			...
			Beam.Size =  Vector3.new(.4 ,.4 ,13)
			Beam.Color = Color3.fromRGB(0,0,0)
			Beam.CFrame = Character.PrimaryPart.CFrame * CFrame.new(math.random(-4,4), math.random(-4,4),-Magnitude/2)
			Beam.Material = "Neon"
			
			Beam.Parent = workspace.World.Visuals

			local Transparency = TweenService:Create(Beam, TweenInfo.new(.35, Enum.EasingStyle.Sine), {Transparency = 1})
			Transparency:Play()
