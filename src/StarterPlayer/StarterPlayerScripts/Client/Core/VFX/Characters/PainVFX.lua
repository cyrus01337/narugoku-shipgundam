...
local function CreateRock(Result)
    ...
    local Tween =
        TweenService:Create(Rock, TweenInfo.new(0.35, Enum.EasingStyle.Linear), { Position = Result.Position })
    Tween:Play()
    Tween:Destroy()
...
local PainVFX = {
    ["AlmightyPush"] = function(PathData)
        ...
        local Magnitude = (Target.PrimaryPart.Position - Character.PrimaryPart.Position).Magnitude
        local Midpoint = Magnitude / 2

        for i = 1, 10 do
            local Beam = HitPart2:Clone()
            Beam.Size = Vector3.new(0.4, 0.4, 13)
            Beam.Color = Color3.fromRGB(0, 0, 0)
            Beam.CFrame = Character.PrimaryPart.CFrame
                * CFrame.new(math.random(-4, 4), math.random(-4, 4), -Magnitude / 2)
            Beam.Material = "Neon"

            Beam.Parent = workspace.World.Visuals

            local Transparency =
                TweenService:Create(Beam, TweenInfo.new(0.35, Enum.EasingStyle.Sine), { Transparency = 1 })
            Transparency:Play()
    ...
    ["bezierballs"] = function(Data)
        ...
        VfxHandler.FakeBodyPart({
            ...
            Object = "Right Arm",
            Material = "Neon",
            Color = script.beizerBalls[Data.WhichBall].Color,
            Transparency = 1,
            Duration = 1,
            Delay = 0.1,
        })
        ...
        VfxHandler.Beam(
            ...
            ColorSequence.new(
                ...
                Color3.fromRGB(script.beizerBalls[Data.WhichBall].Color)
            ),
            1,
            15,
            50,
            nil,
            NumberSequence.new(0.8, 1)
        )
        ...
        coroutine.resume(coroutine.create(function()
            ...
            local Connection
            Connection = BallTing.Rocket.ReachedTarget:Connect(function()
                ...
                Explosions.bezierballs({
                    Color = Color,
                    Character = Character,
                    Position = BallTing.Position,
                    Ball = BallTing,
    ...
    ["boingzz"] = function(Data)
        ...
        VfxHandler.FakeBodyPart({
            ...
            TweenColor = Color3.fromRGB(136, 136, 136),
            Transparency = 1,
            Duration = 1,
            Delay = 0.1,
        })
        ...
        coroutine.resume(coroutine.create(function()
            ...
            VfxHandler.Beam(
                ...
                0,
                0.25,
                ColorSequence.new(BrickColor.new("Pastel light blue").Color, Color3.fromRGB(255, 255, 255)),
                1,
                15,
                50,
                nil,
    ...
    ["cast shield"] = function(Data)
        ...
        TaskScheduler:AddTask(0.25, function()
        ...
        for Index = 1, 2 do
            ...
            local Tween = TweenService:Create(
                Ball,
                TweenInfo.new(2.35, Enum.EasingStyle.Elastic, Enum.EasingDirection.Out),
    ...
    ["brekapart"] = function(Data)
        ...
        TaskScheduler:AddTask(0.75, function()
            Smoke.Enabled = false
        end)

        Debris:AddItem(Smoke, 1 + 0.2)
    ...
    ["shamac~hit"] = function(Data)
        ...
        VfxHandler.FakeBodyPart({
            ...
            Object = "Left Leg",
            Material = "Glass",
            Color = Color3.fromRGB(177, 66, 52),
            TweenColor = Color3.fromRGB(83, 78, 136),
            Transparency = 1,
            Duration = 4,
            Delay = 0.1,
        })
        VfxHandler.FakeBodyPart({
            ...
            Object = "Right Arm",
            Material = "Glass",
            Color = Color3.fromRGB(165, 35, 35),
            TweenColor = Color3.fromRGB(100, 94, 165),
            Transparency = 1,
            Duration = 4,
            Delay = 0.1,
        })
        VfxHandler.FakeBodyPart({
            ...
            TweenColor = Color3.fromRGB(66, 47, 93),
            Transparency = 1,
            Duration = 4,
            Delay = 0.1,
        })
    ...
    ["Push"] = function(Data)
        ...
        coroutine.resume(coroutine.create(function()
            while os.clock() - StartTime < 0.5 do
                if os.clock() - LastRock >= RockInBetween then
                    ...
                    local LeftRay = RayService:Cast(
                        Left,
                        Left * CFrame.new(0, -4, 0),
                        { workspace.World.Visuals, workspace.World.Live },
                        Enum.RaycastFilterType.Exclude
                    )
                    local RightRay = RayService:Cast(
                        Right,
                        Right * CFrame.new(0, -4, 0),
                        { workspace.World.Visuals, workspace.World.Live },
                        Enum.RaycastFilterType.Exclude
                    )
        ...
        local Tween = TweenService:Create(
            Swirl,
            Tweeninfo,
            { Size = Vector3.new(20, 24, 20), CFrame = Swirl.CFrame * CFrame.new(0, 24 / 2, 0) }
        )
        Tween:Play()
        Tween:Destroy()

        Debris:AddItem(Swirl, 0.5)
        TaskScheduler:AddTask(0.027, function()
            for Index = 1, 2 do
                ...
                local Tweeninfo2 = TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, 0, false, 0)
        ...
        TaskScheduler:AddTask(0.3, function()
        ...
        while Swirl do
            ...
            RunService.RenderStepped:Wait()
            if workspace.World.Visuals:FindFirstChild(Character.Name .. " beatrice push swirl") == nil then
                break
            end
        end
    end,
    ...
    ["Al~Shamac"] = function(Data)
        ...
        coroutine.resume(coroutine.create(function()
            for Index = 1, 40 do
                for _ = 1, math.random(1, 2) do
                    local StartPosition = (
                        Vector3.new(
                            math.sin(360 * Index) * math.random(5, 10),
                            0,
                            math.cos(360 * Index) * math.random(5, 10)
                        ) + Root.Position
                    )
                    ...
                    if RaycastResult then
                        local partHit, pos, normVector =
                            RaycastResult.Instance or nil, RaycastResult.Position or nil, RaycastResult.Normal or nil
                        if partHit then
                            local Block = ReplicatedStorage.Assets.Effects.Meshes.Block:Clone()

                            local X, Y, Z =
                                math.random(20, 50) / 100, math.random(20, 50) / 100, math.random(20, 50) / 100
        ...
        FakeCharacter.HumanoidRootPart.CFrame = Root.CFrame * CFrame.new(0, 0, -0.3)
        ...
        for _, VictimParts in ipairs(Character:GetChildren()) do
            if
                VictimParts.ClassName == "MeshPart"
                or VictimParts.ClassName == "Part" and VictimParts.Name ~= "HumanoidRootPart"
            then
        ...
        Yami.Yami.Drag = 0
        Yami.Yami.Lifetime = NumberRange.new(0.35)
