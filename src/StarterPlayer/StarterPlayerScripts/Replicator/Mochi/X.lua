...
local function mochiExplosion(OriginPosition, OriginInstance)
    ...
    for i = 1, amount do
        ...
        local rayCheck = CFrame.new(OriginPosition)
            * CFrame.new(Random.new():NextNumber(-20, 20), 15, Random.new():NextNumber(-20, 20))

        local roofCheckRay = Ray.new(rayCheck.Position, Vector3.new(0, 50, 0))
        local Rhit, Rvec2Pos, RsurfaceNormal =
            workspace:FindPartOnRayWithIgnoreList(roofCheckRay, { mochiPart, debrisFolder })
...
local function groundCrack(rayPos)
    ...
    if hit then
        ...
        crack.CFrame = CFrame.new(vec2Pos, vec2Pos + surfaceNormal)
            * CFrame.Angles(-math.pi / 2, math.random(360), 0)
            * CFrame.new(0, crack.Size.Y / 2, 0)
        ...
        TS:Create(
            ...
            { Size = Vector3.new(crackFinalSize, 0.001, crackFinalSize) }
        ):Play()
...
return function(TargetPosition)
    ...
    local originPosition = (CFrame.new(TargetPosition) * CFrame.new(
    ...
    TS:Create(
        ...
        { Size = Vector3.new(23, 23, 13) * 0.33 }
    ):Play()
    TS:Create(
        mochiRollDetail,
        TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut),
        { Size = Vector3.new(13, 13, 0) * 0.33 }
    ):Play()
    TS:Create(
        mochiRollClone,
        TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
        { Size = Vector3.new(15, 8, 6) * 2, Transparency = 1 }
    ):Play()
    ...
    task.delay(0.7, function()
        TS:Create(
            mochiRoll,
            TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut),
            { Size = Vector3.new(0, 0, 13) * 0.33 }
        ):Play()
        TS:Create(
            mochiRollDetail,
            TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut),
            { Size = Vector3.new(0, 0, 25) * 0.33 }
        ):Play()
        game.Debris:AddItem(mochiRoll, 0.2)
        game.Debris:AddItem(mochiRollDetail, 0.25)
        game.Debris:AddItem(mochiRollClone, 0.1)
    end)
    ---                      ---

    task.delay(0.1, function()
        mochiArm.CFrame = CFrame.lookAt(originPosition, TargetPosition)
            * CFrame.new(0, 0, -0.5)
            * CFrame.Angles(math.rad(180), 0, 0)
        mochiArm.Outline.CFrame = CFrame.lookAt(originPosition, TargetPosition)
            * CFrame.new(0, 0, -0.5)
            * CFrame.Angles(math.rad(180), 0, 0)
        ...
        TS
            :Create(
                ...
                {
                    Size = Vector3.new(2, 2, (originPosition - TargetPosition).Magnitude * 2),
                    CFrame = mochiArm.CFrame * CFrame.new(0, 0, ((originPosition - TargetPosition).Magnitude * 2) / 2),
                }
            )
            :Play()
        TS
            :Create(
                ...
                TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut),
                {
                    Size = Vector3.new(2, 2, (originPosition - TargetPosition).Magnitude * 2) * 1.05,
                    CFrame = mochiArm.CFrame * CFrame.new(0, 0, ((originPosition - TargetPosition).Magnitude * 2) / 2),
                }
            )
            :Play()
        TS:Create(
            ...
            TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut),
            {
                Size = Vector3.new(11, 11, 25) * 0.33,
                CFrame = CFrame.lookAt(originPosition, TargetPosition) * CFrame.new(0, 0, -2.5),
            }
        ):Play()
        ...
        task.delay(0.25, function()
            ...
            task.delay(0.2, function()
                TS:Create(
                    ...
                    { Size = Vector3.new(0, 0, mochiArm.Size.Z) }
                ):Play()
                TS:Create(
                    mochiArm.Outline,
                    TweenInfo.new(0.15, Enum.EasingStyle.Back, Enum.EasingDirection.In),
                    { Size = Vector3.new(0, 0, mochiArm.Outline.Size.Z) }
                ):Play()
