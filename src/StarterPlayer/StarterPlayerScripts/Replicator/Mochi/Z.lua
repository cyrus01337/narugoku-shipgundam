...
local function mochiImpactBall(OriginPosition, originalSize)
    ...
    if hit then
        ...
        TS:Create(
            mochiFloor,
            TweenInfo.new(spawnTime, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
            { Size = Vector3.new(targetSize.X * 1.25, math.random(1, 3), targetSize.Z * 1.25) }
        ):Play()
        TS:Create(
            mochiFloor,
            TweenInfo.new(tDur, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
        ...
        task.delay(spawnTime, function()
            TS:Create(
                mochiFloor,
                TweenInfo.new(tDur - 0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                { Size = Vector3.new(targetSize.X * 1.1, 1, targetSize.Z * 1.1) }
            ):Play()
        end)
    ...
    TS:Create(
        ...
        TweenInfo.new(tDur, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
        {
            Orientation = Vector3.new(math.random(-30, 15), math.random(-179, 179), math.random(-50, 15)),
            Color = Color3.fromRGB(255, 246, 208),
        }
    ):Play()
    ...
    task.delay(Random.new():NextNumber(4, 5), function()
        TS
            :Create(
                mochiBall,
                TweenInfo.new(Random.new():NextNumber(0.45, 0.7), Enum.EasingStyle.Quad, Enum.EasingDirection.InOut),
                { Position = mochiBall.Position - Vector3.new(0, 3, 0), Size = Vector3.new(0, 0, 0) }
            )
            :Play()
    ...
    if Hit.Value == true then
        ...
        repeat
            ...
            if trail == 0 then
                ...
                cloneFX.CFrame = projectile.CFrame
                    * CFrame.new(Random.new():NextNumber(-3.5, 3.5), Random.new():NextNumber(-3.5, 3.5), 0)
                cloneFX.Size = Vector3.new(14, 14, 14)

                cloneFX.Parent = debrisFolder

                TS:Create(cloneFX, TweenInfo.new(0.1), { Size = cloneFX.Size * 0.9 }):Play()
                task.delay(0.1, function()
                    TS
                        :Create(
                            cloneFX,
                            TweenInfo.new(
                                Random.new():NextNumber(0.8, 1),
                                Enum.EasingStyle.Quad,
                                Enum.EasingDirection.Out
                            ),
                            { Size = Vector3.new(0, 0, 0), Color = Color3.fromRGB(255, 246, 208) }
                        )
                        :Play()
            ...
            if dripChance < 5 then
                ...
                local hit, vec2Pos, surfaceNormal =
                    workspace:FindPartOnRayWithIgnoreList(ray, { Character, debrisFolder })
                ...
                if hit then
                    ...
                    TS:Create(
                        dripFX,
                        TweenInfo.new(tDuration, Enum.EasingStyle.Linear),
                        { CFrame = endCF, Color = Color3.fromRGB(255, 246, 208) }
                    ):Play()
                    task.delay(tDuration, function()
                        TS
                            :Create(
                                dripFX,
                                TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut),
                                { Size = Vector3.new(poolSize * 2.5, 0.7, poolSize * 2.5) }
                            )
                            :Play()
                        task.delay(5, function()
                            TS
